...
"""
Summarize.py
Tree-KG 阶段一·步骤2：自底向上摘要（支持 .md / .docx 源）
- 读取 toc_structure.json（含每个节点的 level、id、title、children）
- 在源文件中定位每个节点的段落范围 para_start/para_end（稳健匹配）
- 叶子节点：抽正文 -> LLM 并发摘要
- 非叶节点：并发聚合子摘要 -> 生成上层摘要
...
# ===== 输入格式 =====
_INPUT_FORMAT = SummarizeConfig.get("INPUT_FORMAT", "docx")
...
def _resolve_input_path() -> Path:
    ...
    if _INPUT_FORMAT == "md":
        name = SummarizeConfig.get("MD_NAME", "")
        if not name:
            raise ValueError("MD 模式下必须配置 MD_NAME")
    else:
        name = SummarizeConfig.get("DOCX_NAME", "")
        if not name:
    ...
    if p.exists():
        return p
    raise FileNotFoundError(f"未找到输入文件：{name}")
...
def make_heading_string(level: int, node_id: str, title: str) -> str:
    """生成用于源文件中检索的标题字符串。MD 模式直接返回标题；DOCX 模式拼接编号。"""
...
# ── MD 模式：段落加载 ──
def load_md_paragraphs(md_path: Path) -> List[str]:
    """逐行读取 md 文件，返回规范化行列表（保留标题标记 #）。"""
    with md_path.open("r", encoding=SummarizeConfig["ENCODING"]) as f:
        lines = f.readlines()
    return [normalize_text(line.rstrip('\n\r')) for line in lines]
...
def find_heading_para(paras: List[str], heading: str) -> Optional[int]:
    """
    在段落列表中查找标题位置。
    MD 模式：匹配以 ## / # 开头且包含 heading 的行。
    DOCX 模式：优先包含匹配；退化为编号前缀匹配。
    """
    h = normalize_text(heading)
    if _INPUT_FORMAT == "md":
        # 精确匹配：行以 # 开头，且去除 # 和空格后与 heading 一致
...
def extract_para_span(paras: List[str], start: int, end: int) -> str:
    ...
    # MD 模式：额外跳过以 # 开头的子标题行（避免摘要混入子节点标题）
    if _INPUT_FORMAT == "md":
...
def run() -> None:
    logger.info("Summarize started. format=%s, input=%s, toc=%s, out=%s",
                _INPUT_FORMAT, input_path.resolve(), toc_json_path.resolve(), out_json_path.resolve())
    ...
    # 加载段落
    if _INPUT_FORMAT == "md":
        paras = load_md_paragraphs(input_path)
    else:
        # DOCX 模式（保留）
        # paras = load_docx_paragraphs(input_path)
        raise NotImplementedError(
            "DOCX 模式暂未启用。如需使用，请取消 Summarize.py 中 load_docx_paragraphs 的注释。"
        )
    logger.info("Loaded paragraphs=%s (format=%s)", len(paras), _INPUT_FORMAT)
...
def main():
    ap = argparse.ArgumentParser(description="自底向上并发摘要（支持 md/docx 源）")
