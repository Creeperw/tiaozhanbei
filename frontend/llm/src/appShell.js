...
const PRIMARY_NAV = [
  ...
  { key: 'training-workshop', label: '训练工坊', children: [{ label: '题目训练', intent: intent('training-workshop', { taskType: 'topic_training' }) }, { label: 'AI 病患模拟', intent: intent('training-workshop', { taskType: 'simulated_patient' }) }, { label: '历史记录', intent: intent('training-workshop', { taskType: 'training_history' }) }, { label: '错题变式', intent: intent('training-workshop', { taskType: 'mistake_variation' }) }, { label: '试卷生成', intent: intent('training-workshop', { taskType: 'paper_generation' }) }] },
  {
    key: 'personalization',
    label: '个人数据',
