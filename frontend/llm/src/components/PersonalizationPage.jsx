...
export default function PersonalizationPage({ onBackHome, onBack, embedded = false, view = 'profile' }) {
    ...
    try {
      ...
      setProfile({
        ...
        diet_restrictions: overviewData.profile?.diet_restrictions || confirmedProfile.time_constraints
          || '',
        ...(learnerData?.resource_preferences !== undefined
          ? { exercise_preferences: learnerData.resource_preferences } : {}),
        ...(learnerData?.time_constraints !== undefined
          ? { diet_restrictions: learnerData.time_constraints } : {}),
      });
      setLearnerProfile({
        locked_fields: [],
        survey: {},
  ...
  const saveProfile = async (nextProfile = profile, nextLearnerProfile = learnerProfile) => {
    ...
    try {
      const learnerResponse = await fetchWithAuth(`${API_BASE}/personalization/learner-profile`, {
          method: 'PUT',
          body: JSON.stringify({
      ...
      if (!learnerResponse.ok) throw new Error('画像保存失败');
