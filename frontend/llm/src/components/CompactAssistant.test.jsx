...
describe('CompactAssistant', () => {
  beforeEach(() => {
    ...
    getAssistantPendingRun.mockResolvedValue(null);
  ...
  it('defers restored message history until the collapsed assistant is expanded', async () => {
    listAssistantSessions.mockResolvedValue([{ id: 'session-saved', title: '方剂学复习' }]);
    loadAssistantMessages.mockResolvedValue([
      { id: 1, role: 'assistant', content: '展开后才加载的方剂学记录。' },
    ]);

    render(<CompactAssistant initiallyCollapsed currentUser="admin" onOpenFull={vi.fn()} />);

    await waitFor(() => expect(listAssistantSessions).toHaveBeenCalledOnce());
    expect(loadAssistantMessages).not.toHaveBeenCalled();

    fireEvent.click(screen.getByRole('button', { name: '展开智能助教' }));

    expect(await screen.findByText('展开后才加载的方剂学记录。')).toBeInTheDocument();
    expect(loadAssistantMessages).toHaveBeenCalledOnce();
    expect(loadAssistantMessages).toHaveBeenCalledWith('session-saved');
  });
  ...
  it('detaches a running stream without showing a false stopped error in a new conversation', async () => {
    streamAssistantMessage.mockImplementationOnce((_sessionId, _content, { signal }) => (
    ...
    const input = await screen.findByRole('textbox', { name: '向智能助教提问' });
    fireEvent.change(input, { target: { value: '请生成长期规划' } });
    fireEvent.click(screen.getByRole('button', { name: '发送问题' }));
    ...
    expect(await screen.findByText(/今天可以继续完成你的学习计划/)).toBeInTheDocument();
    await waitFor(() => expect(screen.queryByText('回答已停止')).not.toBeInTheDocument());
    expect(screen.getByRole('textbox', { name: '向智能助教提问' })).toBeEnabled();
  });

  it('unlocks the composer when a restored background run is no longer pending', async () => {
    listAssistantSessions.mockResolvedValue([{ id: 'session-running', title: '后台任务' }]);
    loadAssistantMessages.mockResolvedValue([
      { id: 1, role: 'assistant', content: '后台任务已经完成。' },
    ]);
    let finishLookup;
    getAssistantPendingRun
      .mockResolvedValueOnce({ status: 'running', runId: 'THREAD_RUNNING' })
    ...
    expect(await screen.findByText('当前会话任务正在后台运行，切换页面不会中断。')).toBeInTheDocument();
    expect(screen.getByRole('textbox', { name: '向智能助教提问' })).toBeDisabled();
    await waitFor(() => expect(finishLookup).toBeTypeOf('function'));

    await act(async () => finishLookup(null));

    await waitFor(() => {
      expect(screen.queryByText('当前会话任务正在后台运行，切换页面不会中断。')).not.toBeInTheDocument();
      expect(screen.getByRole('textbox', { name: '向智能助教提问' })).toBeEnabled();
    });
  });
