      ...
      {loading ? (
        ...
        <div className="textbook-chapter-learning__body">
          <aside className="textbook-learning-nav" aria-label="课程导航">
            ...
            <button type="button" className={!pageNotesOpen && courseMode === 'graph' ? 'is-active' : ''} onClick={() => { setPageNotesOpen(false); setCourseMode('graph'); setSectionExamMode(false); }}><Network aria-hidden="true" size={18} />知识图谱</button>
          </aside>
            ...
            {courseMode === 'pdf' ? (
              ...
              <section className="textbook-knowledge-graph" aria-label="中医知识图谱">
                <div className="textbook-learning-main__toolbar">
                  <div><h2>中医知识图谱</h2><p>Tree-KG · 中医学基础</p></div>
                  <button type="button" className="textbook-open-pdf" onClick={() => setCourseMode('pdf')}><BookOpen aria-hidden="true" size={15} />退出全屏</button>
                </div>
                {knowledgeGraphUrl ? (
