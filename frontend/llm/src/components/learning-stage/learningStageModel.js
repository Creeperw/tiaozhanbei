...
export function plannedStagesFromPath(payload) {
  const nodes = Array.isArray(payload?.nodes) ? payload.nodes : [];
  return nodes
    .filter((node) => node?.node_type === 'stage')
    .sort((left, right) => Number(left.order || 0) - Number(right.order || 0))
    .map((node, index) => {
      const preset = DEFAULT_LEARNING_STAGES[index % DEFAULT_LEARNING_STAGES.length];
      const bookCount = Math.max(0, Number(node.child_count || 0));
      return {
        id: String(node.node_id),
        nodeId: String(node.node_id),
        level: statusLabels[node.status] || `第 ${index + 1} 阶段`,
        title: String(node.title || `第 ${index + 1} 阶段`),
        duration: bookCount > 0 ? `${bookCount} 本教材` : '教材待规划',
        tasks: [String(node.description || '阶段目标待补充')],
        resources: [],
        status: node.status || 'unassessed',
        illustration: preset?.illustration || '',
        illustrationPosition: preset?.illustrationPosition,
      };
    });
}
