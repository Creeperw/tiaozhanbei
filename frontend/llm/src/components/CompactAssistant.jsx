...
const CompactMarkdown = React.memo(({ content }) => (
  <div className="compact-assistant__markdown">
    <ReactMarkdown
...
const BACKGROUND_RUN_NOTICE = '当前会话任务正在后台运行，切换页面不会中断。';
...
export default function CompactAssistant({
  ...
  const hydratingSessionRef = useRef(null);
    ...
    const initialize = async () => {
      ...
      try {
        ...
        if (restoredId) {
          ...
          hydratedSessionRef.current = fresh.sessionId;
        }
      } catch (loadError) {
  ...
  useEffect(() => {
    ...
    loadAssistantMessages(sessionId)
      ...
      .finally(() => {
        if (hydratingSessionRef.current === sessionId) hydratingSessionRef.current = null;
        if (!cancelled && sessionGenerationRef.current === generation) setLoading(false);
      });
    return () => {
      cancelled = true;
      if (hydratingSessionRef.current === sessionId) hydratingSessionRef.current = null;
  ...
  useEffect(() => {
    ...
    const syncBackgroundRun = async () => {
      try {
        ...
        if (!pending) {
          setSending(false);
          setError(current => current === BACKGROUND_RUN_NOTICE ? '' : current);
          return;
        }
        if (['running', 'cancellation_requested'].includes(pending.status)) {
          setSending(true);
          setError(BACKGROUND_RUN_NOTICE);
          timer = window.setTimeout(syncBackgroundRun, 2000);
          return;
        }
        setSending(false);
        setError(current => current === BACKGROUND_RUN_NOTICE ? '' : current);
        if (['completed', 'interrupted', 'waiting_human_review', 'cancelled'].includes(pending.status)) {
  ...
  const send = async () => {
    ...
    try {
      if (!activeSessionId) {
        ...
        // A just-created server session has no history to hydrate. Mark it as
        // current before changing state so the deferred-history effect cannot
        // race with and erase the first streamed turn.
      ...
      if (sessionGenerationRef.current === generation) {
        setSending(false);
        if (abortRef.current === streamController) abortRef.current = null;
      }
    }
  };
        ...
        {visibleMessages.map((message) => (
          <div key={message.id} className={`compact-assistant__message is-${message.role}`}>
            ...
            {message.role === 'assistant' ? (
              <div className="compact-assistant__body">
                <CompactMarkdown content={message.content || '正在思考…'} />
              </div>
            ) : (
