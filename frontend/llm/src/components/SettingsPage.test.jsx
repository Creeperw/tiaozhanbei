...
describe('SettingsPage state model', () => {
  ...
  it('keeps standalone user settings free of notification preference controls', () => {
    const { container } = render(<SettingsPage />);

    expect(container).toBeEmptyDOMElement();
    expect(screen.queryByRole('heading', { name: '通知与主动提醒' })).not.toBeInTheDocument();
    expect(screen.queryByRole('button', { name: '保存更改' })).not.toBeInTheDocument();
    expect(fetchWithAuth).not.toHaveBeenCalled();
  });

  it('shows loading, dirty and saving states for embedded notification preferences', async () => {
    ...
    fireEvent.click(screen.getByLabelText('启用站内通知'));
