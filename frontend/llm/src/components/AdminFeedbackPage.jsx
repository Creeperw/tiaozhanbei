...
import { EvolutionGovernancePanel, PreferenceTrainingPanel } from './EvolutionGovernancePanel';

const emptyEdit = { feedback_type: '', rating: '', reason: '', user_feedback: '', question: '', answer: '', metadata: {} };

const LegacyFeedbackTable = () => {
...
const AdminFeedbackPage = () => {
  ...
  return <div className="admin-feedback-page text-slate-800">
    <div className="mx-auto mb-5 flex max-w-7xl gap-2 rounded-2xl border border-emerald-100 bg-white p-2 shadow-sm">
      {tabs.map(([key, label]) => <button key={key} onClick={() => setTab(key)} className={`rounded-xl px-4 py-2 text-sm font-semibold ${tab === key ? 'bg-emerald-600 text-white' : 'text-slate-600 hover:bg-emerald-50'}`}>{label}</button>)}
    </div>
    <div className="mx-auto max-w-7xl">
      {tab === 'evolution' && <EvolutionGovernancePanel />}
      {tab === 'training' && <PreferenceTrainingPanel />}
    </div>
    {tab === 'feedback' && <LegacyFeedbackTable />}
  </div>;
};
