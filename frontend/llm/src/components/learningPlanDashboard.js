...
export async function loadTextbookLearningSummary(book, route = 'textbook_14_5', { signal } = {}) {
  const normalizedBook = normalizeBookName(book);
  if (!normalizedBook) throw new Error('教材名称不能为空');
  ...
  const chapters = Array.isArray(chapterPayload?.nodes) ? chapterPayload.nodes : [];
  const totalSections = chapters.reduce((sum, chapter) => {
    const count = Number(chapter?.children_count);
  ...
  const completedSections = Math.min(totalSections, completedSectionIds.size);
  const progress = totalSections > 0 ? completedSections / totalSections : null;
  const history = Array.isArray(progressPayload?.history) ? progressPayload.history : [];
  const latestHistory = [...history].sort((a, b) => historyTimestamp(b) - historyTimestamp(a))[0] || null;
...
const textbookSummaryCache = new Map();
...
export async function loadCachedTextbookLearningSummary(
  book,
  route = 'textbook_14_5',
  { signal, cacheKey = 'anonymous', force = false } = {},
) {
  const target = snapshotTarget({ name: book, navigation: { route_id: route } }, cacheKey);
  const cached = textbookSummaryCache.get(target.key);
  if (!force && cached?.data && Date.now() - cached.updatedAt < TEXTBOOK_SNAPSHOT_FRESH_MS) {
    return cached.data;
  }
  try {
    const summary = await loadTextbookLearningSummary(target.name, target.route, { signal });
    textbookSummaryCache.set(target.key, { data: summary, updatedAt: Date.now() });
    return summary;
  } catch (error) {
    if (cached?.data && error?.name !== 'AbortError') return cached.data;
    throw error;
    ...
    const load = async () => {
      ...
      const worker = async () => {
        while (nextIndex < refreshTargets.length && !controller.signal.aborted) {
          ...
          nextIndex += 1;
          try {
            const snapshot = await loadCachedTextbookLearningSummary(target.name, target.route, {
            ...
            if (!textbookSummaryCache.get(target.key)?.data) {
              setSnapshots((current) => ({
                ...current,
                byBook: {
                  ...current.byBook,
