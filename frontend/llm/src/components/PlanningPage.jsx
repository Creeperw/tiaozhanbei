...
export default function PlanningPage() {
  ...
  useEffect(() => {
    if (!['queued', 'running'].includes(latestReview?.execution_status)) return undefined;
    const timer = window.setInterval(loadPlan, 2500);
    return () => window.clearInterval(timer);
  }, [latestReview?.execution_status, loadPlan]);
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-[28px] bg-[#f2f8f4] p-5 sm:p-6" aria-label="规划自动复盘">
        ...
        {latestReview ? <article className="mt-5 rounded-2xl bg-white p-4"><div className="flex flex-wrap items-center justify-between gap-3"><strong className="text-sm text-slate-950">{latestReview.summary}</strong><span className="text-xs text-slate-500">{latestReview.period_key}</span></div><div className="mt-3 flex flex-wrap gap-2 text-xs text-slate-600">{(latestReview.evidence || []).map((item) => <span key={item} className="rounded-lg bg-slate-100 px-2 py-1">{item}</span>)}</div>{['queued', 'running'].includes(latestReview.execution_status) && <p role="status" className="mt-3 rounded-xl bg-amber-50 px-3 py-2 text-xs text-amber-800">{latestReview.execution_status === 'queued' ? '重规划已排队…' : '正在更新短期计划和今日任务…'}</p>}{latestReview.execution_status === 'succeeded' && <p role="status" className="mt-3 rounded-xl bg-emerald-50 px-3 py-2 text-xs text-emerald-800">短期计划和今日任务已更新。</p>}{latestReview.execution_status === 'failed' && <p role="alert" className="mt-3 rounded-xl bg-rose-50 px-3 py-2 text-xs text-rose-700">{latestReview.execution?.error || '重规划执行失败，可重试。'}</p>}{(latestReview.status === 'proposal_pending' || latestReview.execution_status === 'failed') && <div className="mt-4 flex gap-2"><button type="button" className="button button--primary" disabled={reviewBusy} onClick={() => decideReview('accept')}>{latestReview.execution_status === 'failed' ? '重试调整' : '接受调整'}</button>{latestReview.status === 'proposal_pending' && <button type="button" className="button button--secondary" disabled={reviewBusy} onClick={() => decideReview('reject')}>保持原计划</button>}</div>}</article> : <div className="mt-5 rounded-2xl bg-white/70 p-4 text-sm text-slate-500">尚无复盘记录。系统会按周生成，或者你可以立即运行一次。</div>}
      </section>
