...
export default function QualificationAttemptWorkspace({ attempt: initialAttempt, onExit }) {
  ...
  const activeFavorite = current ? {
    ...
    title: `真题模拟 · ${String(current.question_content || '').slice(0, 80)}`,
    content: {
  ...
  if (report) {
    ...
    const savedQuestion = reportItem && originalItem ? {
      ...
      defaultTitle: `真题模拟 · 第 ${reportItem.position} 题`,
      content: {
  ...
  return (
    <section className="relative mx-auto max-w-[87.5%] min-h-[560px] overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">">
      ...
      <article className="mx-auto max-w-3xl px-5 py-8 sm:py-10">
        <div className="flex items-start gap-3 text-lg font-medium leading-8 text-slate-950"><span className="mt-0.5 flex h-7 min-w-7 items-center justify-center rounded-full bg-emerald-100 px-2 text-sm font-bold text-emerald-800">{position}</span><span className="min-w-0 flex-1">{current.question_content}</span></div>
