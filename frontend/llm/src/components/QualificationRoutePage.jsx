  ...
  return (
    <div className="home-plan" aria-label="当前学习计划">
      ...
      <section className="home-today-card" aria-label="今日任务" data-task-count={activeItems.length}>
        ...
        {activeTaskKind === 'learning' && currentTask?.scheduling?.summary && (
          <p className="home-today-card__scheduling" role="status">
            {currentTask.scheduling.summary}
          </p>
        )}
...
function HomeLearningRoute({
  ...
  const autoExpandCurrentStageRef = useRef(false);
  const [autoExpandingStage, setAutoExpandingStage] = useState(false);
  ...
  useEffect(() => {
    ...
    autoExpandCurrentStageRef.current = false;
  ...
  // 当前正在查看教材层的阶段：教材节点（下一级）在 orbit 中展示时，
  // 通过父级 ID 反查阶段；仍处于阶段层时取当前（进行中/下一/首个）阶段。
  const viewingStage = useMemo(() => {
    if (routeState.loading || routeState.error) return null;
    if (routeState.nodes.length > 0 && routeState.nodes.every((node) => node.node_type === 'book')) {
      const parentId = String(
        routeState.nodes[0]?.parent_id
        || routeState.nodes[0]?.parent_membership_id
        || '',
      );
      if (parentId) {
        const matched = routeState.stages.find((stage) => (
          String(stage.node_id || stage.nodeId || stage.id) === parentId
        ));
        if (matched) return matched;
      }
    }
    const active = routeState.nodes.find((node) => node.status === 'in_progress')
      || routeState.nodes.find((node) => node.status === 'next')
      || routeState.nodes[0];
    return active || null;
  }, [routeState.error, routeState.loading, routeState.nodes, routeState.stages]);

  // 默认视图直接进入当前阶段的下一级学习路径（教材层），
  // 不再停留在阶段圆点；展开期间显示加载状态而不是阶段层图。
  useEffect(() => {
    if (routeState.loading || routeState.error || routeState.stages.length === 0) return;
    if (routeState.nodes.some((node) => node.node_type !== 'stage')) return;
    if (autoExpandCurrentStageRef.current) return;
    autoExpandCurrentStageRef.current = true;
    const currentStage = routeState.stages.find((stage) => ['in_progress', 'current'].includes(stage.status))
      || routeState.stages[0];
    if (currentStage) {
      setAutoExpandingStage(true);
      openNode(currentStage).finally(() => setAutoExpandingStage(false));
    }
  }, [routeState.error, routeState.loading, routeState.nodes, routeState.stages]);

  const showLearningStages = () => changeRouteView('cards');

  const selectStagePath = async ({ stage }) => {
    await openNode(stage);
    changeRouteView('orbit');
  };
  ...
  return (
      ...
      {renderedRouteView === 'orbit' && (
        <div className="home-portal__route-orbit-layout">
          ...
          {!routeState.loading && !routeState.error && routeState.stages.length > 0 && (
            autoExpandingStage && routeState.nodes.length > 0
            && routeState.nodes.every((node) => node.node_type === 'stage') ? (
              <div className="home-portal__route-state" aria-live="polite">
                正在加载教材学习路径…
              </div>
            ) : (
            <>
              ...
              <div className="home-portal__route-current-stage">
                <div className="home-portal__route-current-stage-copy">
                  <span className="home-portal__route-current-stage-label">当前所在阶段</span>
                  <strong className="home-portal__route-current-stage-name">
                    {viewingStage?.title || '尚未开始'}
                  </strong>
                </div>
                <button
                  type="button"
                  className="home-portal__route-current-stage-action"
                  onClick={showLearningStages}
      ...
      {renderedRouteView === 'cards' && (
        routeState.loading ? (
          <div className="home-portal__route-state" aria-live="polite">
            正在读取{routeMode === 'personalized' ? '个性化' : '经典'}学习阶段…
          </div>
        ) : routeState.error ? (
          emptyPathContent || <div className="home-portal__route-state" role="alert">{routeState.error}</div>
        ) : routeState.stages.length === 0 ? (
          emptyPathContent || <div className="home-portal__route-state">尚未生成学习阶段</div>
        ) : (
          <LearningStageLanding
            compact
            compactTitle="选择阶段查看教材学习路径"
            stages={routeState.stages}
            currentStageId={String(viewingStage?.node_id || viewingStage?.nodeId || viewingStage?.id || '')}
            onStageSelect={selectStagePath}
...
export default function QualificationRoutePage({ currentUser, onNavigate }) {
  ...
  const initialTarget = initialPageCache?.learningTarget || null;
  const initialRouteKey = initialTarget
    ? initialTarget.textbook_route_id || plannedRouteKey(initialTarget)
    : '';
  const hasInitialRoute = Boolean(
    initialRouteKey
    && readQualificationRouteCache(routeCacheKey(userCacheKey, initialRouteKey)),
  );
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (selected) {
        ...
        updatePageCache(userCacheKey, { learningTarget: null });
      }
    }).catch(() => {}).finally(() => {
  ...
  if (learningTargetReady && !learningTarget) {
    return (
      <div className="home-portal" data-page-revealed="true" aria-busy="false">
        <section className="home-portal__learning-area" aria-label="学习路线与学习进度">
          <div className="home-portal__personalized-empty" role="status">
            <span className="home-portal__personalized-empty-icon"><Sparkles aria-hidden="true" size={28} /></span>
            <h1>请先选择资格考试</h1>
