...
export default function DashboardPage({
  ...
  const snapshotBooks = useMemo(() => {
    if (!plannedBooks.length) return currentStageBooks;
    return visibleTextbooks;
  }, [currentStageBooks, plannedBooks.length, visibleTextbooks]);
  ...
  useEffect(() => {
    ...
    const controller = new AbortController();
    const route = (
      allTextbooks.find((book) => normalizedBookName(book) === currentBookName)?.navigation?.route_id
      || currentPlanBook?.navigation?.route_id
      || 'textbook_14_5'
    );
    loadCachedTextbookLearningSnapshot(currentBookName, route, {
      signal: controller.signal,
      cacheKey: teachingResourcesCacheKey,
    })
      .then((detail) => {
        if (!controller.signal.aborted) setCurrentBookDetail(detail);
      })
      .catch(() => {});
    return () => controller.abort();
  }, [allTextbooks, currentBookName, currentPlanBook, teachingResourcesCacheKey]);
  const currentBookSnapshot = (
    currentBookDetail?.book === currentBookName
      ? currentBookDetail
      : learningMetrics.snapshots.byBook[currentBookName] || null
  );
