...
import { loadCachedTextbookLearningSummary } from '../learningPlanDashboard';
import {
...
vi.mock('../learningPlanDashboard', () => ({
  loadCachedTextbookLearningSummary: vi.fn(),
  normalizeBookName: (value) => String(value || '').replace(/[《》]/g, '').trim(),
}));
...
describe('classic route progress projection', () => {
  ...
  it('keeps backend-selected book as current and earlier books as completed', () => {
    const books = applyProgressToBookNodes(
      [
        { node_id: 'b1', node_type: 'book', title: '《中医学基础》', status: 'completed', navigation: { book: '中医学基础' } },
        { node_id: 'b2', node_type: 'book', title: '《中医文化学》', status: 'in_progress', navigation: { book: '中医文化学' } },
        { node_id: 'b3', node_type: 'book', title: '《中国医学史》', status: 'locked', navigation: { book: '中国医学史' } },
      ],
      {},
    );

    expect(books.map((book) => book.status)).toEqual(['completed', 'in_progress', 'next']);
    expect(books.map((book) => book.completed_count)).toEqual([1, 0, 0]);
    expect(books.map((book) => book.incomplete_count)).toEqual([0, 1, 1]);
  });

  it('loads lightweight summaries without forcing the cache on route mount', async () => {
    loadCachedTextbookLearningSummary.mockResolvedValue({ progress: 0 });

    await loadClassicRouteProgress(baseState(), { concurrency: 1 });

    expect(loadCachedTextbookLearningSummary).toHaveBeenCalledTimes(4);
    expect(loadCachedTextbookLearningSummary).toHaveBeenCalledWith(
      '中医学基础',
      'textbook_14_5',
