...
describe('LearningPathOverview', () => {
  ...
  it('renders a closed ordered orbit, progress center, and sequential path segments', () => {
    ...
    expect(screen.getByLabelText('总体学习进度 54%')).toBeInTheDocument();
  ...
  it('shows near-zero overall progress when the active stage has no mastery yet', () => {
    const freshNodes = [
      { membership_id: 'basics', title: '中医基础理论', child_count: 8, status: 'in_progress', order: 1 },
      { membership_id: 'formula', title: '方剂学', child_count: 8, status: 'locked', order: 2 },
      { membership_id: 'clinic', title: '中医内科学', child_count: 6, status: 'locked', order: 3 },
    ];
    render(
      <LearningPathOverview
    ...
    // 刚开始学习（无 average_mastery / progress）时不应凭空出现进度。
    expect(screen.getByLabelText('总体学习进度 0%')).toBeInTheDocument();
  });
