import {
  loadCachedTextbookLearningSummary,
  normalizeBookName,
...
export function applyProgressToBookNodes(nodes, snapshotsByBook = {}) {
  ...
  // 优先信任后端投影给出的状态（来自规划中的“本周期选中教材”），
  // 快照进度只作为补充：已完成或部分进度仍可参与排序。
  const backendCurrentIndex = nodes.findIndex((node) => node.status === 'in_progress');
  const statuses = progressValues.map((progress, index) => (
    progress === 1 || nodes[index]?.status === 'completed' ? 'completed' : 'locked'
  ));
  const activeIndex = progressValues.findIndex((progress) => progress != null && progress > 0 && progress < 1);
  const currentIndex = backendCurrentIndex >= 0 && statuses[backendCurrentIndex] !== 'completed'
    ? backendCurrentIndex
    : activeIndex >= 0
    ? activeIndex
    : statuses.findIndex((status) => status !== 'completed');
  if (currentIndex >= 0) {
    statuses[currentIndex] = 'in_progress';
    if (currentIndex + 1 < statuses.length && statuses[currentIndex + 1] !== 'completed') {
      statuses[currentIndex + 1] = 'next';
    }
  }
  return nodes.map((node, index) => {
    ...
    return {
      ...
      completed_count: progress === 1 || node.status === 'completed' ? 1 : 0,
      incomplete_count: progress === 1 || node.status === 'completed' ? 0 : 1,
...
async function loadSnapshotsForBookNodes(
  ...
  const worker = async () => {
    while (nextIndex < bookNodes.length && !signal?.aborted) {
      ...
      if (!book) continue;
      try {
        snapshotsByBook[book] = await loadCachedTextbookLearningSummary(
          book,
...
export async function loadPlannedRouteProgress(
  routeState,
  { signal, userCacheKey = 'anonymous', force = false, concurrency = 4 } = {},
) {
...
export async function loadClassicRouteProgress(
  ...
  const worker = async () => {
    while (nextIndex < books.length && !signal?.aborted) {
      ...
      nextIndex += 1;
      try {
        snapshotsByBook[book] = await loadCachedTextbookLearningSummary(book, atlasRouteId, {
          signal,
