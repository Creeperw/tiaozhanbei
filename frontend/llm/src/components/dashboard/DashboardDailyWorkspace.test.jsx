...
describe('DashboardDailyWorkspace', () => {
  ...
  it('puts the path title and source controls in one header group', () => {
    render(
      <DashboardDailyWorkspace
        showFocus={false}
        greeting="你好"
        ...
        pathControls={(
          <div role="tablist" aria-label="学习路径来源">
            <button type="button" role="tab" aria-selected="true">我的学习路径</button>
            <button type="button" role="tab" aria-selected="false">经典路线</button>
          </div>
        )}
        pathContent={<section>路径</section>}
        assistantContent={<aside>助教</aside>}
  ...
  it('uses one right rail and switches between today task and assistant without losing state', () => {
    render(
      <DashboardDailyWorkspace
        showFocus={false}
        greeting="你好"
