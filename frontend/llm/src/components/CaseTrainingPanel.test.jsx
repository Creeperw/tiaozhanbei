...
describe('CaseTrainingPanel', () => {
  beforeEach(() => {
    ...
    globalThis.fetch = vi.fn().mockResolvedValue({ ok: true, text: async () => JSON.stringify({
  ...
  it('starts a simulated-patient session through the unified API', async () => {
    ...
    expect(globalThis.fetch).toHaveBeenCalledWith('/api/v1/simulated-patient', expect.objectContaining({ method: 'POST' }));
  });
});
