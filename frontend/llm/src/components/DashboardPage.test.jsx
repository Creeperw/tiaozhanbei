...
describe('DashboardPage replacement learning workshop', () => {
  ...
  it('does not load section pages for textbooks outside the current plan', async () => {
    loadAtlasNodes.mockImplementation(({ level, lv1 }) => Promise.resolve({
      route: 'textbook_14_5',
      nodes: level === 1
        ? [
            { id: 'atlas-book-1', name: '中医学基础' },
            { id: 'atlas-book-2', name: '中医文化学' },
          ]
        : level === 2
          ? [{ id: `chapter-${lv1}`, name: '第一章', children_count: 2 }]
          : [{ id: `section-${lv1}`, name: '第一节' }],
    }));

    render(<DashboardPage onNavigate={vi.fn()} />);

    await screen.findByRole('region', { name: '教材学习列表' });
    await waitFor(() => expect(loadAtlasNodes).toHaveBeenCalledWith(expect.objectContaining({
      level: 3,
      lv1: '中医学基础',
    })));
    expect(loadAtlasNodes).not.toHaveBeenCalledWith(expect.objectContaining({
      level: 3,
      lv1: '中医文化学',
    }));
  });
