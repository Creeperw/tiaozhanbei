...
export default function LearningActivityHeatmap({ series = [] }) {
  ...
  return (
    <section className="rounded-[28px] border border-emerald-100 bg-white p-5 shadow-sm shadow-emerald-950/5" aria-label="学习活跃度">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 text-lg font-bold text-slate-950">学习活跃度</div>
        </div>
        <span className="rounded-full bg-emerald-50 px-3 py-1.5 text-sm font-medium text-emerald-800">{totalActiveDays} 个活跃日</span>
      </div>
        ...
        <div className="grid w-full min-w-0 grid-cols-[20px_repeat(12,minmax(0,1fr))] gap-0.5 sm:grid-cols-[24px_repeat(12,minmax(0,1fr))] sm:gap-1">
          ...
          {WEEKDAY_LABELS.map((weekday, dayIndex) => (
            <React.Fragment key={weekday}>
              <div className="flex h-6 min-w-0 items-center text-[10px] text-slate-400 sm:text-xs">{dayIndex % 2 ? '' : weekday}</div>
              {weeks.map((week) => {
                ...
                const title = item
                  ...
                  : `${key}：暂无学习记录`;
                return <span key={key} title={title} aria-label={title} className={`h-6 min-w-0 rounded-[5px] ${activityTone(score)}`} />;
              })}
            </React.Fragment>
          ))}
        </div>
