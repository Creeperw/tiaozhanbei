...
`TEXTBOOK_TOC_EXTRACTION_FAILED`。正文只交给 MinerU，超过 180 页的 PDF 会自动分片处理并
合并 Markdown 与真实 PDF 页码，不要求用户预先拆书。
...
视频原子项的 `resource_ref.duration_seconds` 是片段真实时长（秒），由视频资源解析器按
`end_seconds - start_seconds` 计算，并随学习路径进度工具一起进入诊断智能体上下文；
智能体据此分配预计时间，物化阶段 `video_section` 项按真实时长向上取整分钟，剩余预算均分
给其余练习原子项，避免“看视频”预估时间与视频实际时长脱节。
前端今日任务点击 `knowledge_practice` 原子项时进入专题训练并锁定对应知识点，
点击 `video_section` 原子项时进入教材章节学习的对应小节，两者都携带 `taskItemId` 与
`returnTo` 以便返回今日任务页。
