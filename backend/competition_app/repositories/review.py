...
class ReviewRepository(Protocol):
    ...
    def update_task_status(self, review_task_id: str, status: str) -> ReviewTask | None: ...

    def postpone_memory_unit(
        self, learner_id: str, kp_id: str, next_review_at: datetime
    ) -> None: ...
...
class InMemoryReviewRepository:
    def __init__(self) -> None:
        ...
        self._daily_capacity: dict[str, int] = {}
        self._lock = RLock()
    ...
    def update_task_status(self, review_task_id: str, status: str) -> ReviewTask | None:
        with self._lock:
            ...
            self._tasks[review_task_id] = updated
            return updated
    ...
    def postpone_memory_unit(
        ...
        with self._lock:
            unit = self._units.get((learner_id, kp_id))
            if unit is None:
                return
            self._units[(learner_id, kp_id)] = unit.model_copy(
                update={
    ...
    def get_latest_schedule(self, learner_id: str) -> ReviewSchedule | None:
        ...
        if not matches:
            return None
        return max(matches, key=lambda item: item.calculated_at)

    def get_daily_capacity(self, learner_id: str) -> int | None:
        with self._lock:
            return self._daily_capacity.get(learner_id)

    def set_daily_capacity(self, learner_id: str, capacity: int) -> None:
        with self._lock:
            self._daily_capacity[learner_id] = int(capacity)
...
class SqlReviewRepository:
    ...
    def update_task_status(self, review_task_id: str, status: str) -> ReviewTask | None:
        ...
        if not payload:
            return None
        task = ReviewTask.model_validate(self._json(payload))
        updated = task.model_copy(update={"status": status})
        with self.engine.begin() as connection:
            connection.execute(
                text(
                ...
                ),
                {
                    ...
                    "payload_json": json.dumps(
                        updated.model_dump(mode="json"), ensure_ascii=False
                    ),
                },
            )
        return updated
    ...
    def postpone_memory_unit(
        ...
        if not current:
            return
        unit = ReviewMemoryUnit.model_validate(self._json(current))
        ...
        with self.engine.begin() as connection:
            connection.execute(
                text(
                    ...
                    "updated_at=CURRENT_TIMESTAMP WHERE learner_id=:learner_id AND kp_id=:kp_id"
                ),
                {
                    "learner_id": learner_id,
                    "kp_id": kp_id,
    ...
    def get_latest_schedule(self, learner_id: str) -> ReviewSchedule | None:
        with self.engine.connect() as connection:
            payload = connection.execute(
                text(
                    "SELECT payload_json FROM review_schedules "
                    "WHERE learner_id=:learner_id "
                    "ORDER BY calculated_at DESC LIMIT 1"
                ),
                {"learner_id": learner_id},
            ).scalar_one_or_none()
        return ReviewSchedule.model_validate(self._json(payload)) if payload else None

    def get_daily_capacity(self, learner_id: str) -> int | None:
        with self.engine.connect() as connection:
            capacity = connection.execute(
                text(
                    "SELECT daily_capacity FROM review_preferences "
                    "WHERE learner_id=:learner_id"
                ),
                {"learner_id": learner_id},
            ).scalar_one_or_none()
    ...
    def set_daily_capacity(self, learner_id: str, capacity: int) -> None:
        with self.engine.begin() as connection:
            existing = connection.execute(
                text(
                    "SELECT learner_id FROM review_preferences "
                    "WHERE learner_id=:learner_id"
                ),
                {"learner_id": learner_id},
            ).first()
            values = {"learner_id": learner_id, "capacity": int(capacity)}
            if existing:
                connection.execute(
                    text(
                        "UPDATE review_preferences SET daily_capacity=:capacity, "
                ...
                connection.execute(
                    text(
                        ...
                        "(learner_id, daily_capacity) VALUES (:learner_id, :capacity)"
                    ),
                    values,
                )
