...
class LangGraphExecutionState(TypedDict):
    ...
    failures: Annotated[dict[str, dict[str, Any]], _merge_mappings]
...
class _InterruptedSession:
    ...
    # 最近活动时间（monotonic 秒），用于过期会话懒清理，防止中断/挂起会话
    # 在内存中无限累积（审核挂起期间会话不释放，是内存暴涨来源之一）。
    last_used_at: float
...
class LangGraphOrchestrator(Orchestrator):
    ...
    # 整个图执行（含所有步骤与澄清循环）的总预算：步骤级已有 timeout_seconds
    # 护栏，这里兜底防极端场景（多次澄清、修复循环等）导致无限挂起。
    # 高并发评测下外部 API 排队，knowledge 步骤已放宽到 1200s，总预算同步调大。
    _GRAPH_TOTAL_BUDGET_SECONDS = DEFAULT_GRAPH_TIMEOUT_SECONDS
    # 中断会话最长保留时长：超过后视为用户放弃该会话，不再支持恢复。
    _SESSION_MAX_AGE_SECONDS = 3600.0
    ...
    def __init__(
        ...
        *,
        ...
        self._pending_checkpoint_deletions: list[asyncio.Task] = []

    def _expire_stale_sessions(self) -> None:
        """Lazily drop interrupted sessions idle for longer than the max age."""
        if not self._interrupted_sessions:
            return
        now = time.monotonic()
        stale = [
            thread_id
            for thread_id, session in self._interrupted_sessions.items()
            if now - session.last_used_at > self._SESSION_MAX_AGE_SECONDS
        ]
        for thread_id in stale:
            session = self._interrupted_sessions.pop(thread_id, None)
            if session is not None:
                try:
                    task = asyncio.create_task(
                        self._checkpointer.adelete_thread(thread_id)
                    )
                    # 保存引用防止任务被 GC；完成即从列表移除，避免无界累积。
                    self._pending_checkpoint_deletions.append(task)
                    task.add_done_callback(
                        self._pending_checkpoint_deletions.remove
                    )
    ...
    def restore_thread(
        ...
        if thread_id in self._interrupted_sessions:
            self._interrupted_sessions[thread_id].last_used_at = time.monotonic()
            return
        plan.validate_dag()
        ...
        *,
        ...
        try:
            state = await asyncio.wait_for(
                ...
                timeout=plan.graph_timeout_seconds,
            )
        except asyncio.TimeoutError:
            # 图级总预算兜底：步骤级超时已保证单步不挂起，这里防极端场景
            # （多次澄清/修复循环）整体超时。超时后清理会话与检查点，避免
            # 挂起执行长期占用内存与数据库线程。
            ...
            return ExecutionResult(
                ...
                error_message=(
                    f"工作流执行超过总预算 "
                    f"({plan.graph_timeout_seconds:.0f}s)，已终止本次处理。"
                ),
        ...
        *,
        ...
        try:
            state = await asyncio.wait_for(
                ...
                timeout=session.plan.graph_timeout_seconds,
            )
        except asyncio.TimeoutError:
            ...
            return ExecutionResult(
                ...
                error_message=(
                    f"工作流恢复执行超过总预算 "
                    f"({session.plan.graph_timeout_seconds:.0f}s)，已终止本次处理。"
                ),
                thread_id=thread_id,
            )
        except Exception as exc:
    ...
    async def abandon_thread(self, thread_id: str) -> None:
        """Drop an interrupted session and its durable checkpoint.

        Used when a resume answer turns out to be a brand-new request that
        must start a fresh run instead of continuing the stale clarify loop.
        """
        self._interrupted_sessions.pop(thread_id, None)
        try:
            await self._checkpointer.adelete_thread(thread_id)
        ...
        *,
        ...
        if failures:
            ...
            return ExecutionResult(
                ...
                error_diagnostics=dict(failure.get("error_diagnostics") or {}),
                thread_id=thread_id,
            )
                ...
                return {
                    ...
                    "failures": {
                        action.step_id: {
                            ...
                            "error_diagnostics": failure_diagnostics(exc),
                        }
                    },
        ...
        async def run_node(
            ...
            record = RepairTrace(
                ...
                preserved_before_digest=self.repair_controller._output_digest({
                    step_id: ({**outputs, **node_outputs}).get(step_id)
    ...
    @staticmethod
    def _memory_conflict_affects_route(
        clarification: dict[str, Any],
        root_context: dict[str, Any],
    ) -> bool:
    ...
    def _clarification_payload(result: Any, step: ExecutionStep) -> dict[str, Any] | None:
        ...
        prerequisite_kind = getattr(
            clarification_source, "prerequisite_kind", None
        )
