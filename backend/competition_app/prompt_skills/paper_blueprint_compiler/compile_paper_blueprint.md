...
`unit_key` 仅按原稿单元顺序建立本次合同内关联，不是数据库 ID。时长、总分或单元分值未写时必须为 `null`，不得计算默认值。
...
6. `duration_minutes`、`total_score`、`required_question_count`、`score_total`
   必须输出 JSON 数字，不得附加“分钟”“分”“题”等单位；对应
