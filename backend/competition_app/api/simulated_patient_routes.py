...
ACUPUNCTURE_CASE_DATA_PATH = Path(__file__).resolve().parents[1] / "data" / "acupuncture_cases.v1.json"

# ── 引擎引用（由 app.py 在启动时注入） ─────────────────
_engine: Optional[SimulatedPatientEngine] = None
_activity_projection: Callable[[str, dict], None] | None = None
...
def init_engine(
    ...
    *,
    ...
    activity_projection: Callable[[str, dict], None] | None = None,
) -> SimulatedPatientEngine:
    """由 app.create_app 在初始化时调用。

    chat_model 参数保留以兼容调用方（当前适配器自行读取配置，无需外部注入）。
    """
    global _engine, _activity_projection
    _activity_projection = activity_projection
...
async def simulated_patient_entry(request: Request, body: SPRequest):
    ...
    if (
        ...
        and _activity_projection is not None
    ):
        try:
            await asyncio.to_thread(
                ...
                {
                    "session_id": body.session_id,
                    "history_id": (result.data or {}).get("history_id"),
                    "practice_scope": body.practice_scope,
                    "data": result.data,
                },
            )
        except Exception:
            # The legacy JSON result remains authoritative for this response;
            # the SQL projection is observability-only and must not break it.
            import logging
            logging.getLogger(__name__).warning(
                "legacy simulated-patient activity projection failed",
                exc_info=True,
            )
    return {
