...
class ReviewService:
    def __init__(
        ...
        *,
        canonical_mapping_provider=None,
    ) -> None:
        self.repository = repository
        self.canonical_mapping_provider = canonical_mapping_provider

    def _canonical_units(self, learner_id: str) -> list[ReviewMemoryUnit]:
        units = self.repository.list_memory_units(learner_id)
        if not units or self.canonical_mapping_provider is None:
            return units
        mapping = self.canonical_mapping_provider(
            learner_id,
            tuple(unit.kp_id for unit in units),
        )
        grouped: dict[str, list[ReviewMemoryUnit]] = {}
        for unit in units:
            grouped.setdefault(mapping.get(unit.kp_id, unit.kp_id), []).append(unit)
        selected = []
        for canonical_id, candidates in grouped.items():
            canonical = next(
                (item for item in candidates if item.kp_id == canonical_id),
                None,
            )
            if canonical is None:
                if len(candidates) != 1:
                    raise ValueError(
                        f"canonical review projection is missing for {canonical_id}"
                    )
                canonical = candidates[0].model_copy(update={"kp_id": canonical_id})
    ...
    def get_queue(
        ...
        *,
        ...
        delivery_mapping = (
            self.canonical_mapping_provider(
                learner_id,
                tuple(item.task.primary_kp_id for item in deliveries),
            )
            if deliveries and self.canonical_mapping_provider is not None
            else {}
        )
        delivery_by_kp = {}
        for delivery in deliveries:
            canonical_id = delivery_mapping.get(
                delivery.task.primary_kp_id,
                delivery.task.primary_kp_id,
            )
            display_delivery = (
                ReviewDelivery(
                    task=delivery.task.model_copy(
                        update={"primary_kp_id": canonical_id}
                    ),
                    resource=delivery.resource,
                )
                if canonical_id != delivery.task.primary_kp_id
                else delivery
            )
            previous = delivery_by_kp.get(canonical_id)
            if previous is None or delivery.task.priority_score > previous.task.priority_score:
                delivery_by_kp[canonical_id] = display_delivery

        entries: list[ReviewQueueEntry] = []
        for unit in self._canonical_units(learner_id):
    ...
    def postpone_due_memory_units(
        ...
        *,
        ...
        """Postpone every due memory unit that has no active review task.

        The learner-facing "复习任务" includes due memory units that have not
        yet been materialized into ``review_tasks`` rows.  Cancelling/snoozing
        must cover those too, otherwise the assistant replies "没有待处理的
        复习任务" while the dashboard still shows due items.  Returns the kp_ids
        that were postponed.
        """
        ...
        for entry in queue.entries:
            ...
            self.repository.postpone_memory_unit(
                ...
                + timedelta(hours=max(1, min(720, int(hours)))),
            )
            postponed.append(entry.memory_unit.kp_id)
        return postponed

    def get_latest_schedule(self, learner_id: str) -> ReviewSchedule | None:
        """Return the learner's most recent persisted ReviewSchedule (if any)."""

        return self.repository.get_latest_schedule(learner_id)

    def get_daily_capacity(self, learner_id: str) -> int | None:
        """Return the learner's persisted review capacity override (if any)."""

        return self.repository.get_daily_capacity(learner_id)

    def set_daily_capacity(self, learner_id: str, capacity: int) -> None:
        """Persist the learner's review capacity override (1..50, safe clamp)."""

        self.repository.set_daily_capacity(learner_id, max(1, min(50, int(capacity))))
    ...
    def cancel_task(
        ...
        *,
        ...
        """Cancel a review task and postpone the underlying memory unit so the
        task is not immediately regenerated. ``postpone_hours`` is clamped to
        1..720 (30 days).
        """
        ...
        self.repository.postpone_memory_unit(
            ...
            task.primary_kp_id,
            datetime.now(timezone.utc)
            + timedelta(hours=max(1, min(720, int(postpone_hours)))),
        )
        updated = self.repository.update_task_status(review_task_id, "cancelled")
        if updated is None:
            raise RuntimeError("review task disappeared while cancelling")
        return updated
    ...
    def snooze_task(
        ...
        *,
        ...
        """Delay a review task without cancelling it: the underlying memory
        unit's next review is pushed back and the task returns to ``pending``
        (out of the due set). ``hours`` is clamped to 1..720 (30 days).
        """
        ...
        updated = self.repository.update_task_status(review_task_id, "pending")
        if updated is None:
            raise RuntimeError("review task disappeared while snoozing")
        return updated
