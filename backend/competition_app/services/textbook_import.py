...
_TOC_CHAPTER_RE = re.compile(r"^\s*(?:第\s*[一二三四五六七八九十百千零0-9]+\s*[章篇]|绪论)")

_TOC_SECTION_RE = re.compile(r"^\s*第\s*[一二三四五六七八九十零0-9]+\s*节")
...
class TextbookImportService:
    ...
    async def import_pdf(
        ...
        *,
        ...
        try:
            ...
            locator, extracted = await self._recognize_toc(pdf_path, document, digest)
            toc_pages = sorted({
                int(page) for page in locator.get("toc_pdf_pages", [])
                if str(page).isdigit() and 1 <= int(page) <= len(document.pages)
            })
            chapters = extracted.get("chapters") if isinstance(extracted, dict) else None
            has_toc = bool(locator.get("has_toc") and toc_pages and isinstance(chapters, list) and chapters)
            toc_status = "extracted" if has_toc else None
            if toc_status is None:
                outline_chapters = self._toc_from_outline(document)
                if outline_chapters:
                    chapters = outline_chapters
                    toc_pages = []
                    toc_status = "outline"
                    self._report(progress, "toc", "未识别到目录页，已改用 PDF 内嵌书签…")
            ...
            if text_layer:
                ...
                markdown_path = await asyncio.to_thread(self._run_markitdown, pdf_path, book_dir / "markitdown")
                ...
                mineru_markdown, page_text = await asyncio.to_thread(self._run_mineru, pdf_path, mineru_dir)
            if toc_status is None:
                heading_chapters = self._toc_from_headings(page_text)
                if heading_chapters:
                    chapters = heading_chapters
                    toc_status = "headings"
                    self._report(progress, "toc", "未识别到目录页，已按正文标题探测目录…")
                else:
                    chapters = [{"title": "全书", "pdf_page": 1, "sections": []}]
                    toc_status = "flat"
            ...
            if toc_status in ("flat", "outline", "headings"):
                ...
                mapped_toc = self._map_toc_pages(
                    ...
                    toc_last_page=max(toc_pages) if toc_pages else 1,
                )
    ...
    def _toc_from_outline(self, document: Any) -> list[dict[str, Any]] | None:
        try:
    ...
    def _toc_from_headings(self, page_text: dict[int, str]) -> list[dict[str, Any]] | None:
        chapters: list[dict[str, Any]] = []
        current: dict[str, Any] | None = None
        for page in sorted(page_text):
            for raw_line in (page_text[page] or "").splitlines():
                line = raw_line.strip().rstrip("。")
                if not line:
                    continue
                if _TOC_CHAPTER_RE.match(line):
                    current = {"title": line[:50], "pdf_page": page, "sections": []}
                    chapters.append(current)
                    if len(chapters) >= 500:
                        return chapters
                elif _TOC_SECTION_RE.match(line) and current is not None:
                    current["sections"].append({"title": line[:50], "pdf_page": page})
