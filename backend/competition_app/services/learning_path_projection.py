...
class LearningPathProjectionService:
    ...
    def page(
        ...
        *,
        ...
        # 仅在阶段晋级时推进，可能滞后于近期重新规划。
        long_selection = plan.textbook_selection
        stage_selection = long_selection if long_selection is not None else selection
        book_selection: TextbookSelectionContext | None = None
        active_stage_id = stage_selection.stage_id if stage_selection is not None else None
        if selection is not None and selection.stage_id == active_stage_id:
            book_selection = selection
        elif stage_selection is not None:
            book_selection = stage_selection
        ...
        book_match = self._find_book(
            parent_id, plan, stages, stage_nodes, route_id, source_refs,
            book_selection,
        )
    ...
    def _stage_nodes(
        ...
        selection: TextbookSelectionContext | None,
        ...
        for index, stage in enumerate(stages):
            ...
            active_stage_id = selection.stage_id if selection is not None else None
            active_index = (
    ...
    def _book_nodes(
        ...
        selected_books = {
            _clean_book(book) for book in (selection.books if selection is not None else []) or []
        }
        first_selected_index = next(
            (
                index
                for index, raw_book in enumerate(stage.book)
                if _clean_book(raw_book) in selected_books
            ),
            None,
        )
        nodes: list[LearningPathNode] = []
        for index, raw_book in enumerate(stage.book):
            ...
            if self.book_knowledge_loader is not None:
                try:
                    ...
                    child_count = 0
            if stage_node.status == "in_progress":
                # 当前阶段：以“本周期选中教材”为当前书；选中之前的书视为已学完，
                # 之后的书待解锁。缺少选中信息时回退为阶段第一本书。
                if first_selected_index is None:
                    status = "in_progress" if index == 0 else "locked"
                elif index < first_selected_index:
                    status = "completed"
                elif index == first_selected_index:
                    status = "in_progress"
                else:
                    status = "locked"
            elif stage_node.status == "completed":
                status = "completed"
            else:
                status = "locked"
            nodes.append(
    ...
    def _find_book(
        ...
        for index, stage in enumerate(stages):
            for raw_book, node in zip(
                stage.book,
                self._book_nodes(
                    plan, stage, stage_nodes[index], route_id, source_refs, selection
                ),
            ):
