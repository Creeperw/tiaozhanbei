    ...
    def resolve(
        ...
        *,
        ...
        if len(embedded_alias_matches) == 1:
            selected_route = embedded_alias_matches[0]
            selected_goal_type = self._normalized(selected_route.goal_type)
            has_same_type_unsupported_match = any(
                self._normalized(route.goal_type) == selected_goal_type
                for route in unsupported_matches
            )
            if not has_same_type_unsupported_match:
                return self._approved_resolution(
                    selected_route, "embedded_alias"
                ).model_copy(
                    update={"goal_type": goal_type, "goal_name": goal_name}
                )
        if len(embedded_alias_matches) > 1:
