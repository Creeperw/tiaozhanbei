...
class DefaultRouteResolverAgent:
    ...
    async def run(
        ...
        if (
            ...
            # The model may correctly select a route that differs from stale
            # profile/plan prose used only to seed ``_extract_goal``.  Bind the
            # textbook catalog to the approved resolution, not to that stale
            # pre-selection name; otherwise a previous “中西医结合” goal can
            # silently attach the integrated textbook route to a newly selected
            # TCM-physician route.
            resolved_goal_name = (
                self._optional_text(resolution.goal_name) or goal_name
            )
            textbook_route = self._textbook_repository.resolve(
                ...
                goal_text=f"{resolved_goal_name} {request}".strip(),
            )
    ...
    def _trusted_learning_target_resolution(
        ...
        # The active target is a persisted workspace identity. Subject names in
        # an open request (for example ``方剂学``) describe focus within that
        # workspace and must not silently replace it. Only a system-owned,
        # structured target-change signal may bypass the persisted identity.
    ...
    def _has_explicit_target_change(cls, context: dict[str, Any]) -> bool:
        # Never infer workspace identity changes from unrestricted user text.
        # The exam selector persists its choice before planning; any future
        # conversational target-switch flow must set this internal flag from a
        # validated structured contract rather than keyword matching.
        return context.get("explicit_target_change") is True
