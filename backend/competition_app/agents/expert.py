...
class ExpertAgent:
    ...
    async def run(self, context: dict[str, Any]) -> AgentEnvelope[ResourceDraft]:
        ...
        all_question_details = list(evidence_pack._question_details)
        question_details = list(all_question_details)
        if task_type != "paper_generation" and evidence_pack.resolved_kp_ids:
            ...
            normalized_topic = self._normalize_direct_topic_text(topic)
            if len(normalized_topic) >= 3:
                selected_question_ids = {
                    item.question_id for item in question_details
                }
                for channel_group in (("bm25",), ("vector", "runtime_vector")):
                    if any(
                        any(
                            channel in item.retrieval.channels
                    ...
                    fallback = next(
                        (
                            item
                            for item in all_question_details
                            if item.question_id not in selected_question_ids
                            and self._is_review_question_type(item.question_type)
                            and normalized_topic
                            in self._normalize_direct_topic_text(item.stem)
                            and any(
                                channel in item.retrieval.channels
                                for channel in channel_group
                            )
                    ...
                    if fallback is not None:
                        question_details.append(fallback)
                        selected_question_ids.add(fallback.question_id)
        model_question_details = (
            question_details
            if task_type == "paper_generation"
            else reserve_question_details(
                [
                    item
                    for item in question_details
        ...
        candidate_catalog = [
            ...
            for item in model_question_details
        ]
        resource_candidates = [
            item
                            ...
                            "candidate_questions": [
                                {
                                    ...
                                    "kp_ids": item["kp_ids"],
                                    "stem": next(
                                        q.stem for q in model_question_details
        ...
        candidate_ids = {item.question_id for item in model_question_details}
    ...
    @staticmethod
    def _normalize_direct_topic_text(value: object) -> str:
