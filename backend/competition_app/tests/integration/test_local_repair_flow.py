...
async def test_repair_agent_receives_only_its_located_instruction_and_previous_output() -> None:
    ...
    repair_trace = result.repair_trace[0]
    assert repair_trace.initial_audit_decision == "revise"
    assert repair_trace.origin_step_ids == ["expert"]
    assert repair_trace.owner_step_ids == ["expert"]
    assert repair_trace.preserved_outputs_unchanged is True
