...
@pytest.mark.asyncio
@pytest.mark.parametrize("role", ["planner_agent", "memory_agent"])
async def test_deepseek_disables_thinking_for_fast_routing_and_memory(role: str) -> None:
    requests: list[httpx.Request] = []
    ...
    assert await client.complete_json(role, {}) == {"status": "ok"}
    request_body = json.loads(requests[0].content)
    # 高频强结构化调用优先保证最终 JSON；Knowledge/Expert/Audit 等内容
    # 智能体仍使用 thinking 并在前端展示推理流。
...
async def test_chat_client_includes_nested_agent_schema_and_common_rules() -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(
            200,
            json={"choices": [{"message": {"content": '{"summary":"诊断摘要"}'}}]},
        )
        ...
        return httpx.Response(
            ...
            json={
                "choices": [
                    {
                        "message": {
                            "content": '{"status":"compiled","contract":{"title":"试卷蓝图"},"issues":null}'
                        }
                    }
                ]
            },
...
@pytest.mark.asyncio
async def test_chat_client_rotates_key_after_explicit_balance_exhaustion() -> None:
    authorization_headers: list[str] = []

    def handler(request: httpx.Request) -> httpx.Response:
        authorization = request.headers["Authorization"]
        authorization_headers.append(authorization)
        if authorization == "Bearer depleted-secret":
            return httpx.Response(429, json={"message": "insufficient balance"})
        return httpx.Response(
            200,
            json={"choices": [{"message": {"content": '{"status":"ok"}'}}]},
        )
    ...
    client = OpenAICompatibleChatModel(
        base_url="https://example.test/v1",
        api_key="depleted-secret",
        api_keys=("depleted-secret", "healthy-secret"),
        model="deepseek-v4-flash",
        transport=httpx.MockTransport(handler),
    )
...
@pytest.mark.asyncio
async def test_chat_client_reports_when_all_keys_are_exhausted_without_leaking_them() -> None:
    calls = 0
    ...
    secrets = ("first-private-secret", "second-private-secret")
    client = OpenAICompatibleChatModel(
        base_url="https://example.test/v1",
        api_key=secrets[0],
        api_keys=secrets,
...
@pytest.mark.asyncio
async def test_chat_client_does_not_retire_key_for_plain_rate_limit(monkeypatch) -> None:
    calls: list[str] = []
    ...
    client = OpenAICompatibleChatModel(
        ...
        api_keys=("primary-secret", "unused-fallback-secret"),
        model="deepseek-v4-flash",
        transport=httpx.MockTransport(handler),
    )

    assert await client.complete_json("planner_agent", {}) == {"status": "ok"}
    assert calls == ["Bearer primary-secret"] * 3
...
@pytest.mark.asyncio
async def test_chat_client_keeps_transient_retry_after_empty_stream_recovery() -> None:
    statuses: list[str] = []

    def handler(request: httpx.Request) -> httpx.Response:
        is_stream = bool(json.loads(request.content).get("stream"))
        if is_stream:
            statuses.append("empty_stream")
            return httpx.Response(
                200,
                text='data: {"choices":[]}\n\ndata: [DONE]\n\n',
                headers={"content-type": "text/event-stream"},
            )
        if statuses == ["empty_stream"]:
            statuses.append("transient_503")
            return httpx.Response(503, json={"message": "temporary unavailable"})
    ...
    assert statuses == ["empty_stream", "transient_503", "success"]
...
async def test_failover_retries_business_invalid_json_and_hides_failed_stream() -> None:
    ...
    assert failover.last_error_details is None


@pytest.mark.asyncio
async def test_transport_diagnostics_are_isolated_between_concurrent_calls() -> None:
    async def handler(request: httpx.Request) -> httpx.Response:
        body = json.loads(request.content)
        rendered = json.dumps(body["messages"], ensure_ascii=False)
        marker = "slow" if "slow" in rendered else "fast"
        if marker == "slow":
        ...
        return httpx.Response(
            ...
            json={
                "choices": [
                    {"message": {"content": json.dumps({"marker": marker})}}
                ]
            },
        )
    ...
    async def invoke(marker: str) -> tuple[str, str, str]:
        result = await client.complete_json(
            "knowledge_base_agent",
            {"payload": {"marker": marker}},
        )
        request_text = json.dumps(
            client.last_request_payload["body"]["messages"],
            ensure_ascii=False,
        )
        return result["marker"], client.last_response_text or "", request_text

    slow, fast = await asyncio.gather(invoke("slow"), invoke("fast"))

    assert slow[0] == "slow" and '"slow"' in slow[1] and "slow" in slow[2]
    assert fast[0] == "fast" and '"fast"' in fast[1] and "fast" in fast[2]


@pytest.mark.asyncio
async def test_transport_connect_error_retries_then_succeeds() -> None:
    """瞬态 ConnectError（代理抖动）应自动重试，而不是直接让组卷失败。"""
    attempts: list[str] = []
...
@pytest.mark.asyncio
async def test_transport_connect_error_exhausts_retries_then_raises() -> None:
    """持续 ConnectError 耗尽重试预算后，仍以 transport_error 抛出。"""
    attempts: list[str] = []
