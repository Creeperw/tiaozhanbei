...
def test_dashboard_lazy_refresh_emits_daily_task_notification(tmp_path: Path) -> None:
    container = ApplicationContainer.build(Settings(mode="stub"), snapshot_root=tmp_path)
    ...
    class FakeHandoff:
        ...
        def load_daily_task_progress(self, _learner_id, _payload):
            return {"status": "pending", "completed_items": 0, "total_items": 1, "items": []}
        ...
        def create_daily_task_refreshed_notification(self, learner_id, *, task_summary=""):
            notified.append((learner_id, task_summary))

    container.backend_handoff_runtime = FakeHandoff()
    client = TestClient(create_app(container))
    registered = client.post(
        "/api/v1/auth/register",
        json={"username": "dashboard-notify", "password": "correct-horse-2026"},
    ).json()
    learner_id = registered["user"]["user_id"]
    now = datetime.now(timezone.utc)
    short_plan = ShortTermPlan(
        plan_id="SHORT_NOTIFY",
        learner_id=learner_id,
        long_term_plan_id="LONG_NOTIFY",
        content="本周学习补气剂。",
        version=1,
        status="active",
        created_at=now,
    ...
    # 任务已超过 24h 滚动窗口：请求 dashboard 时应触发懒刷新并通知。
    task = LearningTask(
        task_id="TASK_NOTIFY",
        learner_id=learner_id,
        short_term_plan_id=short_plan.plan_id,
        ...
        created_at=now,
        updated_at=now,
        refresh_started_at=now - timedelta(hours=26),
        refresh_due_at=now - timedelta(hours=2),
    )
    container.review_card_use_case.plan_repository.save_current(
        learner_id,
        LearningPlanResult(short_term_plan=short_plan, learning_task=task),
    ...
    assert response.status_code == 200
    assert response.json()["daily_task_timer"]["refreshed"] is True
    assert len(notified) == 1
    assert notified[0][0] == learner_id
    assert isinstance(notified[0][1], str)

    # 同日第二次请求不应重复发通知（backend_handoff 层按 dedupe_key 去重，
    # 且懒刷新只会在任务再次到期时触发）。
    second = client.get("/api/v1/dashboard/home")
    assert second.status_code == 200
    assert second.json()["daily_task_timer"]["refreshed"] is False
    assert len(notified) == 1
