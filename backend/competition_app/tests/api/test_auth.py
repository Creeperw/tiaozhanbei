...
def test_stream_registers_owned_run_before_background_execution_starts(
    tmp_path: Path,
) -> None:
    """The status endpoint must not expose the SSE startup registration race."""

    container = ApplicationContainer.build(Settings(mode="stub"), snapshot_root=tmp_path)
    app = create_app(container)
    alice_client = TestClient(app)
    bob_client = TestClient(app)
    alice = register(alice_client, "runstartupowner")
    register(bob_client, "runstartupintruder")
    thread_id = "THREAD_AUTH_STARTUP_2026"

    # Exercise the route-level boundary directly: the run must be owned and
    # queryable before StreamingResponse starts iterating its event source.
    route = next(
        route
        for route in app.routes
        if getattr(route, "path", None) == "/api/v1/review-cards/stream"
    )
    request = ReviewCardRequest(
        thread_id=thread_id,
        learner_id="untrusted-input",
        user_request="说明四君子汤的组成",
    )
    http_request = Request(
        {
            "type": "http",
            "method": "POST",
            "path": "/api/v1/review-cards/stream",
            "headers": [],
            "app": app,
        }
    )
    http_request.state.current_user = SimpleNamespace(
        user_id=alice["user_id"],
        username=alice["username"],
        role="learner",
    )

    response = asyncio.run(route.endpoint(request, http_request))
    ...
    assert alice_client.get(f"/api/v1/review-cards/runs/{thread_id}").status_code == 200
    assert bob_client.get(f"/api/v1/review-cards/runs/{thread_id}").status_code == 404
