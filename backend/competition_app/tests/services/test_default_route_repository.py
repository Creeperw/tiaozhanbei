...
def test_supported_exam_is_not_shadowed_by_an_unsupported_course_alias(
    repository: DefaultRouteRepository,
) -> None:
    result = repository.resolve(
        goal_type="credential",
        goal_name="中医执业医师资格考试，重点学习方剂学",
    )

    assert result.planning_status == "approved_route"
    assert result.route_id == "tcm_physician_standard_degree"
    assert result.match_reason == "embedded_alias"


def test_repository_keeps_formula_course_unavailable_as_a_standalone_goal(
    repository: DefaultRouteRepository,
) -> None:
    result = repository.resolve(goal_type="course", goal_name="方剂学")
