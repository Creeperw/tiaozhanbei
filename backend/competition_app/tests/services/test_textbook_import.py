...
@pytest.mark.asyncio
async def test_textbook_import_returns_stable_identifier_when_toc_missing(tmp_path: Path):
    service = FakeTextbookImportService(
        runtime_root=tmp_path,
        chat_base_url="https://example.test/v1",
        chat_model="vision-model",
    ...
    with pytest.raises(TextbookTocNotFound) as error:
        await service.import_pdf(
            owner_id="U1",
            filename="无目录教材.pdf",
            content=sample_pdf(),
        )
    assert error.value.code == TOC_EXTRACTION_FAILED
