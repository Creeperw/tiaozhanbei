...
def _state(
    ...
    *,
    ...
    learner_id: str = "learner-daily-refresh",
) -> LearningPlanResult:
...
def test_background_scan_refreshes_due_tasks_in_each_exam_workspace() -> None:
    now = datetime(2026, 7, 23, 8, tzinfo=timezone.utc)
    repository = InMemoryLearningPlanRepository()
    learner_id = "learner-multi-exam"
    callbacks: list[tuple[str, str]] = []

    for exam_track_id in ("EXAM_TCM", "EXAM_PHARMACY"):
    ...
    result = DailyTaskRefreshService(repository).refresh_due_tasks(
        now=now,
        on_refreshed=lambda learner, scope: callbacks.append((learner, scope)),
    )

    assert result == {"scanned": 2, "refreshed": 2, "failed": 0}
    assert callbacks == [
        (learner_id, "EXAM_PHARMACY"),
        (learner_id, "EXAM_TCM"),
    ]
    for exam_track_id in ("EXAM_TCM", "EXAM_PHARMACY"):
        token = bind_exam_workspace_context(
        ...
        try:
            task = repository.get_current(learner_id).learning_task
        finally:
            reset_exam_workspace(token)
        assert task.version == 2
        assert task.refresh_due_at == now + timedelta(hours=24)
