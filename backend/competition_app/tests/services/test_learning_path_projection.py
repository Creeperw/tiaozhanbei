...
def _selection(books: list[str]) -> TextbookSelectionContext:
    return TextbookSelectionContext(
        ...
        books=books,
        reason="依据已批准教材路线确定本周期教材。",
    )


def test_book_status_follows_current_selection() -> None:
    service = LearningPathProjectionService(_loader)
    books = service.page(
        learner_id="LEARNER_TEST",
        plan=_plan(),
        parent_id="plan:LP_TEST:stage:stage-1",
        selection=_selection(["《医古文》"]),
    )
    assert [node.title for node in books.nodes] == ["《中医学基础》", "《医古文》"]
    assert books.nodes[0].status == "completed"
    assert books.nodes[1].status == "in_progress"
    assert books.current_node_id == books.nodes[1].node_id


def test_book_status_falls_back_to_first_book_without_selection() -> None:
    service = LearningPathProjectionService(_loader)
    books = service.page(
        learner_id="LEARNER_TEST",
        plan=_plan(),
    ...
    assert books.nodes[0].status == "in_progress"
    assert books.nodes[1].status == "locked"
    assert books.current_node_id == books.nodes[0].node_id


def test_cross_stage_short_term_selection_does_not_advance_long_term_path() -> None:
    plan = _plan().model_copy(update={"textbook_selection": _selection(["《中医学基础》"])})
    short_term_selection = TextbookSelectionContext(
        route_id="textbook_tcm_physician",
        route_version=1,
        stage_id="stage-2",
        stage_name="方剂辨析",
        books=["《方剂学》"],
        reason="同路线临时入门预习，不代表长期阶段完成。",
    )

    root = LearningPathProjectionService(_loader).page(
        learner_id="LEARNER_TEST",
        plan=plan,
        selection=short_term_selection,
    )

    assert [node.status for node in root.nodes] == ["in_progress", "locked"]
    assert root.current_node_id == "plan:LP_TEST:stage:stage-1"
