...
def test_execution_plan_rejects_model_step_not_above_provider_deadline() -> None:
    plan = ExecutionPlan(
        plan_id="PLAN_DEADLINE_INVALID",
        task_type="knowledge_explanation",
        provider_timeout_seconds=1800.0,
        steps=[
            ExecutionStep(
                step_id="audit",
                agent="audit_agent",
    ...
    with pytest.raises(PlanValidationError, match="must exceed provider timeout"):
        plan.validate_deadlines()


def test_execution_plan_rejects_step_not_below_graph_deadline() -> None:
    plan = ExecutionPlan(
        plan_id="PLAN_GRAPH_DEADLINE_INVALID",
        task_type="knowledge_explanation",
        graph_timeout_seconds=2100.0,
        steps=[
            ExecutionStep(
                step_id="audit",
                agent="audit_agent",
    ...
    with pytest.raises(PlanValidationError, match="below graph timeout"):
        plan.validate_dag()
