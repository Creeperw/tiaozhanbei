...
def test_model_agent_context_has_uniform_metadata_and_business_payload() -> None:
    ...
    assert parsed.prompt_skill_version == "1.17.0"
...
def test_downstream_agent_receives_new_memory_candidates() -> None:
    """Memory Agent 本轮新提取的候选必须注入下游 agent 上下文。"""
    from competition_app.contracts.base import ArtifactReference
    from competition_app.contracts.memory import LongTermMemoryCandidate

    source_refs = [
        ArtifactReference(
            ref_type="conversation_message", ref_id="local-user-1"
        )
    ]
    memory_output = type(
        "MemoryOutput",
    ...
    context = {
        "trace_id": "TRACE_NEW_MEM",
        "request_id": "REQ_NEW_MEM",
        "learner_id": "USER_1",
        "user_request": "帮我记住：我喜欢用表格学习方剂学",
    ...
    candidates = value["payload"]["shared_context"]["new_memory_candidates"]
    assert candidates == [
        {
            "summary": "用户偏好：方剂学学习更喜欢表格形式",
            "status": "pending_confirmation",
...
def test_new_memory_candidates_empty_without_memory_dependency() -> None:
    context = {
        "trace_id": "TRACE_NO_MEM",
        "request_id": "REQ_NO_MEM",
        "learner_id": "USER_1",
        "user_request": "随便聊聊",
    }
    ...
    assert value["payload"]["shared_context"]["new_memory_candidates"] == []
