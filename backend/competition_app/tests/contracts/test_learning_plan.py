...
def test_short_term_focus_context_keeps_only_system_owned_selection_facts() -> None:
    ...
    assert focus.model_dump(exclude_none=True, exclude_defaults=True) == {
        "focus_type": "due_review",
        "focus_label": "四君子汤",
        "knowledge_point_ids": ["KP_FJ_001"],
    }
...
def test_legacy_short_term_focus_json_parses_without_overlay_fields() -> None:
    focus = ShortTermFocusContext.model_validate_json(
        '{"focus_type":"knowledge_point","focus_label":"四君子汤",'
        '"knowledge_point_ids":["KP_FJ_001"]}'
    )

    assert focus.mode is None
    assert focus.progression_stage_id is None
    assert focus.focus_stage_id is None
    assert focus.focus_books == []
    assert focus.focus_names == []
    assert focus.focus_evidence == []
    assert focus.prerequisite_mode is None
