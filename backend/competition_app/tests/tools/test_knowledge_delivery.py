...
def test_canonical_release_paths_do_not_require_legacy_directory_names(
    tmp_path: Path,
) -> None:
    release = tmp_path / "assets" / "knowledge" / "releases" / "release-7"
    paths = KnowledgeDeliveryPaths.from_release_root(
        release,
        runtime_root=tmp_path / "runtime" / "knowledge",
        public_vector_store=tmp_path / "assets" / "vectors" / "release-7",
    )

    assert paths.component_root == (release / "component").resolve()
    assert paths.public_data == (
        release / "component" / "data" / "backend_delivery"
    ).resolve()
    assert paths.video_results == (
        release / "video" / "full_batch_results"
    ).resolve()
...
async def test_delivery_evidence_preserves_retrieval_text_and_local_video(tmp_path: Path) -> None:
    ...
    assert detail["questions"][0]["question_id"] == "Q_PUBLIC"
    assert detail["videos"][0]["bvid"] == "BV_TEST"


@pytest.mark.asyncio
async def test_delivery_evidence_does_not_materialize_full_question_or_video_maps(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    backend = build_backend(tmp_path)

    def reject_full_materialization() -> None:
        raise AssertionError("evidence retrieval must use targeted resource lookup")

    monkeypatch.setattr(backend.map, "ensure_questions", reject_full_materialization)
    monkeypatch.setattr(backend.map, "ensure_videos", reject_full_materialization)

    pack = await backend.build_local_evidence_pack("四君子汤")

    assert any(item.resource_type == "textbook" for item in pack.evidence_items)
    assert any(item.resource_type == "video" for item in pack.evidence_items)


def test_external_question_search_reserves_bm25_and_vector_before_final_limit(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    backend = build_backend(tmp_path)
    ...
    def raw_item(question_id: str, channels: list[str], score: float) -> dict:
        return {
            "question": {
            ...
            "knowledge_points": [],
            "retrieval": {"channels": channels, "score": score},
        }

    ranked = [
        raw_item("BRIDGE_1", ["bridge"], 1.0),
        raw_item("BRIDGE_2", ["bridge"], 0.99),
        raw_item("BRIDGE_3", ["bridge"], 0.98),
        raw_item("BM25", ["bm25"], 0.7),
        raw_item("VECTOR", ["runtime_vector"], 0.6),
    ]

    def search(*args):
        observed_limits.append(int(args[4]))
        return {
            "query": {"resolved_kps": [{"kp_id": "KP_1"}]},
            "embedding_model": "test",
            "items": ranked,
        }

    monkeypatch.setattr(backend, "_module", lambda _name: SimpleNamespace(search=search))
    monkeypatch.setattr(backend, "_sync_embedder", lambda: object())

    result = backend._search_questions(
        "四君子汤", ["KP_1"], 3, None, "public"
    )

    assert observed_limits == [20]
...
def test_trusted_video_resolver_unique_section_title_matches_published_segment(
    tmp_path: Path,
) -> None:
    """D3：小节标题唯一匹配已发布可信视频片段时允许解析。"""
    backend = build_backend(tmp_path)

    resolved = backend.map.resolve_trusted_video_resource(
        {"learning_chapter": "四君子汤"}
    )

    assert resolved is not None
    assert resolved["kp_id"] == "KP_1"
    assert resolved["bvid"] == "BV_TEST"
    assert resolved["section"] == "四君子汤"
    assert resolved["duration_seconds"] == 30
    # 查询无法命中任何已知小节标题时仍拒绝
    assert backend.map.resolve_trusted_video_resource(
        {"learning_chapter": "不存在的知识点名称"}
    ) is None


def test_trusted_video_resolver_ambiguous_section_title_is_rejected(
    tmp_path: Path,
) -> None:
    """D3：小节标题命中多个已发布视频知识点时不唯一，拒绝解析。"""
    backend = build_backend(tmp_path)
    backend.map.ensure_hierarchy()
    backend.map.ensure_videos()
    backend.map.kps["KP_AMBIG"] = {
        "kp_id": "KP_AMBIG",
        "kp_lv1": "方剂学",
        "kp_lv2": "补益剂",
        "kp_lv3": "四君子汤详解",
    ...
    backend.map.videos_by_kp["KP_AMBIG"] = [
        {
            "bvid": "BV_TEST",
            "aid": None,
            "cid": None,
            ...
            "topic": "组成",
            "transcript": "四君子汤详解。",
            "match": {"kp_id": "KP_AMBIG", "confidence": 0.9},
        }
    ]

    # "四君子汤" 同时命中 KP_1 与 KP_AMBIG 的小节标题 → 不唯一 → 拒绝
    assert backend.map.resolve_trusted_video_resource(
        {"learning_chapter": "四君子汤"}
    ) is None
