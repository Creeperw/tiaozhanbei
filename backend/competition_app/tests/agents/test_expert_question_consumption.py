...
@pytest.mark.asyncio
async def test_expert_restores_direct_topic_hit_from_bm25_after_bad_kp_bridge() -> None:
    context = context_with_candidates()
    knowledge = context["dependency_outputs"]["knowledge"].payload
    knowledge._question_details.append(
        QuestionDetail(
            question_id="Q_DIRECT_TOPIC",
            question_type="填空题",
            stem="四君子汤的组成药物包括人参、白术、茯苓和____。",
            reference_answer="甘草",
            analysis="题干直接考查四君子汤。",
            tags=["错误标注知识点"],
            source_metadata={},
    ...
    assert [item["question_id"] for item in model.candidates] == [
        "Q_1",
        "Q_DIRECT_TOPIC",
    ]
...
@pytest.mark.asyncio
async def test_expert_reserves_bm25_and_vector_candidates_at_model_boundary() -> None:
    context = context_with_candidates()
    knowledge = context["dependency_outputs"]["knowledge"].payload

    def candidate(question_id: str, channels: list[str]) -> QuestionDetail:
        return QuestionDetail(
            question_id=question_id,
            question_type="单项选择题",
            stem=f"四君子汤题目 {question_id}",
            reference_answer="A",
            analysis="解析",
            tags=["四君子汤"],
            source_metadata={},
            bridges=[
                QuestionBridge(
                    ...
                    confidence=1.0,
                    rank=1,
                    evidence_chunk_uid=f"C_{question_id}",
                    match_method="strict",
                )
            ],
            retrieval=QuestionRetrievalMetadata(
                channels=channels,
                channel_scores={channel: 0.8 for channel in channels},
                fusion_score=0.8,
            ),
        )

    knowledge._question_details.extend(
        [candidate(f"Q_BRIDGE_{index}", ["bridge"]) for index in range(1, 8)]
        + [candidate("Q_BM25_RESERVED", ["bm25"])]
        + [candidate("Q_VECTOR_RESERVED", ["vector"])]
    )
    model = CaptureCandidateCatalogModel()

    await ExpertAgent(model).run(context)

    candidate_ids = [item["question_id"] for item in model.candidates]
    assert len(candidate_ids) == 8
    assert "Q_BM25_RESERVED" in candidate_ids
    assert "Q_VECTOR_RESERVED" in candidate_ids
...
def test_expert_filters_non_safe_review_question_types() -> None:
    ...
    assert ExpertAgent._is_review_question_type("填空题") is True
