...
def test_single_choice_candidate_with_two_text_answers_is_rejected() -> None:
    question = (
        _assembly_context()["dependency_outputs"]["question_pool"]
        .payload.units[0].items[0]
        .model_copy(
            update={
                ...
                "reference_answer": "阴阳属性的规定性；阴阳的相关性",
                "analysis": "原始题目同时给出两个答案。",
            }
        )
    )

    assert not PaperAssemblyAgent._question_solution_ok(question, _DifficultyUnit())


def test_single_choice_candidate_accepts_one_label_or_exact_option_text() -> None:
    question = (
        _assembly_context()["dependency_outputs"]["question_pool"]
        .payload.units[0].items[0]
        .model_copy(
            update={
                "options": ["A. 对立制约", "B. 互根互用"],
                "reference_answer": "B",
            }
        )
    )

    assert PaperAssemblyAgent._question_solution_ok(question, _DifficultyUnit())
    assert PaperAssemblyAgent._question_solution_ok(
        question.model_copy(update={"reference_answer": "互根互用"}),
        _DifficultyUnit(),
    )


def test_candidate_catalog_snapshot_is_deterministic_and_hides_internal_ids() -> None:
    context = _assembly_context()
    blueprint = context["dependency_outputs"]["paper_blueprint"].payload
    pool = context["dependency_outputs"]["question_pool"].payload

    first, first_view = PaperAssemblyAgent._build_candidate_catalog_snapshot(
        context=context,
        blueprint=blueprint,
        candidate_pool=pool,
        require_explanation=False,
    )
    second, second_view = PaperAssemblyAgent._build_candidate_catalog_snapshot(
        context=context,
        blueprint=blueprint,
        candidate_pool=pool,
        require_explanation=False,
    ...
    assert first.candidates[0].question_id == "Q1"
    assert first_view == second_view
    assert first_view[0]["candidates"][0]["candidate_no"] == 1
    assert "question_id" not in first_view[0]["candidates"][0]
    assert "unit_id" not in first_view[0]
...
class EmptyAssemblyModel(AssemblyModel):
    """候选池为空且模型完全无法补足时的空态模型。"""
...
class SystemBoundSelectionModel:
    async def complete_json(self, role, payload, on_delta=None):
        return {
            "selection_summary": "从系统目录选择第一道合规候选题。",
            "selected_candidates": [
                {"candidate_no": 1, "rationale": "覆盖当前蓝图单元。"}
            ],
        }


class RejectingAssemblyCompiler:
    async def compile(
        self,
        context,
        *,
        assembly_document,
        candidate_catalog,
        generated_unit_binding=None,
    ):
        raise AssertionError("new system-bound path must not invoke legacy LLM compiler")
...
async def test_paper_assembly_falls_back_to_candidate_pool_on_invalid_protocol() -> None:
    ...
    assert any("确定性组装" in item for item in result.payload.assembly_notes)
    assert result.payload.unresolved_constraints == []


@pytest.mark.asyncio
async def test_system_bound_selection_does_not_call_legacy_llm_compiler() -> None:
    context = _assembly_context()
    blueprint = context["dependency_outputs"]["paper_blueprint"].payload
    blueprint.required_total_question_count = None
    blueprint.question_count_is_hard_constraint = False
    ...
    result = await PaperAssemblyAgent(
        SystemBoundSelectionModel(),
        assembly_compiler=RejectingAssemblyCompiler(),
    ).run(context)

    assert [item.question.question_id for item in result.payload.items] == ["Q1"]
    assert result.payload.final_coverage_summary.official_count == 1
    binding = result.payload.items[0].question.source_metadata[
        "paper_candidate_binding"
    ]
    assert binding["binding_id"].startswith("PCB_")
    assert "paper_candidate_binding" not in result.payload.learner_questions()[
        0
...
@pytest.mark.asyncio
async def test_paper_assembly_returns_empty_state_feedback_when_pool_empty() -> None:
    """候选池为空且模型也无法补足时，返回空态反馈而不是抛校验错误。"""
    context = _assembly_context()
    context["dependency_outputs"]["question_pool"].payload.units[0].items = []

    result = await PaperAssemblyAgent(EmptyAssemblyModel()).run(context)

    assert result.payload.items == []
    assert result.payload.empty_reason
    assert "暂未找到与当前学习范围匹配的题目" in result.payload.empty_reason
    assert any(result.payload.empty_reason in item for item in result.payload.unresolved_constraints)
...
@pytest.mark.asyncio
async def test_paper_assembly_ignores_obsolete_model_selection_and_uses_formal_pool() -> None:
    context = _assembly_context()
    blueprint = context["dependency_outputs"]["paper_blueprint"].payload
    blueprint.required_total_question_count = None
    blueprint.question_count_is_hard_constraint = False
    ...
    assert [item.question.question_id for item in result.payload.items] == ["Q1"]
    assert not any("Q_NOT_IN_POOL" in item for item in result.payload.assembly_notes)
    assert any("确定性组装" in item for item in result.payload.assembly_notes)
    assert result.payload.unresolved_constraints == []


@pytest.mark.asyncio
async def test_paper_assembly_selects_26_formal_questions_without_model_call() -> None:
    class SelectionMustNotCallModel:
        async def complete_json(self, role, payload, on_delta=None):
            raise AssertionError("正式候选充足时不得调用模型")

    context = _assembly_context()
    blueprint = context["dependency_outputs"]["paper_blueprint"].payload
    blueprint.required_total_question_count = 26
    blueprint.units[0].required_question_count = 26
    pool = context["dependency_outputs"]["question_pool"].payload
    base = pool.units[0].items[0]
    pool.units[0].required_question_count = 26
    pool.units[0].items = [
        base.model_copy(
            update={
                "question_id": f"Q{index:02d}",
                "stem": f"正式候选题{index:02d}",
            }
        )
        for index in range(1, 27)
    ]

    result = await PaperAssemblyAgent(SelectionMustNotCallModel()).run(context)

    assert len(result.payload.items) == 26
    assert all(item.question.origin == "retrieved" for item in result.payload.items)
    assert [item.question.question_id for item in result.payload.items] == [
        f"Q{index:02d}" for index in range(1, 27)
    ]
    assert any("未调用模型" in note for note in result.payload.assembly_notes)
...
async def test_paper_assembly_discards_different_ids_with_the_same_stem() -> None:
    ...
    assert [item.question.question_id for item in result.payload.items] == ["Q1"]
    assert any("题干重复" in item for item in result.payload.assembly_notes)
    assert result.payload.unresolved_constraints == []
...
@pytest.mark.asyncio
async def test_final_coverage_is_recomputed_after_generated_gap_fill() -> None:
    context = _assembly_context()
    blueprint = context["dependency_outputs"]["paper_blueprint"].payload
    blueprint.required_total_question_count = 2
    ...
    assert paper.final_coverage_summary.total_questions == 2
    assert paper.final_coverage_summary.generated_count == 1
    ...
    assert paper.coverage_summary == paper.final_coverage_summary.model_dump(
        mode="json"
    )
