...
async def test_diagnosis_fetches_bounded_planning_context_after_routing() -> None:
    ...
    registry.register(
        ...
        lambda external_user_id, scope, available_minutes=None, **_: {
...
@pytest.mark.asyncio
async def test_diagnosis_passes_unknown_available_minutes_without_defaulting() -> None:
    registry = ToolRegistry()
    ...
    def planning_context(
        ...
        **_: object,
        ...
        observed["available_minutes"] = available_minutes
        return {
            "source": "authorized_tool",
            "task_load_policy": {"recommended_minutes": available_minutes},
        }
    ...
    result = await DiagnosisAgent()._with_authorized_planning_context(
        {
            "learner_id": "USER_UNKNOWN_BUDGET",
    ...
    assert observed["available_minutes"] is None
    assert result["task_load_policy"] == {"recommended_minutes": None}
