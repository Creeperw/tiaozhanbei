...
@pytest.mark.asyncio
async def test_textbook_binding_uses_selected_route_not_stale_profile_goal(
    repository: DefaultRouteRepository,
    textbook_repository: TextbookRouteRepository,
) -> None:
    model = RouteDecisionModel(
    ...
    result = await DefaultRouteResolverAgent(
    ...
    ).run(
        {
            **agent_context(),
            "plan_scope": "long_term",
            "user_request": "请重新制定并替换当前考试的长期学习规划",
            # Simulate stale prose from another exam-scoped plan. It may help
            # the model interpret context, but must not override its approved
            # route selection when the textbook route is bound.
            "user_profile": {
                "learning_goal": "中西医结合执业医师资格考试",
            },
        }
    )
...
@pytest.mark.asyncio
async def test_route_agent_accepts_supported_exam_with_formula_study_focus(
    repository: DefaultRouteRepository,
    textbook_repository: TextbookRouteRepository,
) -> None:
    model = RouteDecisionModel(
    ...
    ).run(
        {
            **agent_context(),
            ...
            "user_request": (
                ...
                "用户补充的具体变化：中医执业医师资格考试，重点学习方剂学。"
            ),
            "latest_resume_answer": "中医执业医师资格考试，重点学习方剂学。",
            "user_profile": {},
        }
    )
...
@pytest.mark.asyncio
async def test_active_exam_treats_formula_course_as_short_term_focus(
    repository: DefaultRouteRepository,
    textbook_repository: TextbookRouteRepository,
) -> None:
    result = await DefaultRouteResolverAgent(
