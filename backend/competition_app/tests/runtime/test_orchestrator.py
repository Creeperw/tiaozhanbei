...
class HumanReviewRepairSequenceAgent(AuditSequenceAgent):
    """First review escalates with an actionable issue, then passes."""

    async def run(self, context):
        decision = self.decisions[min(self.calls, len(self.decisions) - 1)]
        self.calls += 1
        return type("Output", (), {"payload": AuditResult(
            audit_result_id=f"AUDIT_HUMAN_REVIEW_{self.calls}",
            decision=decision,
            structured_findings=(
                [
                    RepairIssue(
                        issue_id="ISSUE_HUMAN_REVIEW",
                        issue_type="content_quality",
                        message="内容需先返修",
                        owner_step_id="expert",
                        affected_step_ids=["expert"],
...
class UnexpectedlyStartedAgent:
    def __init__(self, events: list[str]) -> None:
        self.events = events

    async def run(self, context):
        self.events.append("unexpected_started")
        return {"ok": True}
...
@pytest.mark.asyncio
async def test_orchestrator_reports_step_deadline_source_and_budget() -> None:
    registry = AgentRegistry()
    registry.register("audit_agent", NeverCompletingAgent())
    plan = ExecutionPlan(
        plan_id="P_AUDIT_TIMEOUT",
    ...
    assert result.status == "failed"
    assert result.error_type == "StepDeadlineExceeded"
...
@pytest.mark.asyncio
async def test_orchestrator_propagates_cancellation_and_does_not_start_next_level() -> None:
    events: list[str] = []
    registry = AgentRegistry()
    blocking = BlockingCancellableAgent(events)
    registry.register("blocking_agent", blocking)
    registry.register("next_agent", UnexpectedlyStartedAgent(events))
    plan = ExecutionPlan(
        plan_id="P_CANCEL_PROPAGATION",
        task_type="personalized_review_card",
        steps=[
            ExecutionStep(step_id="blocking", agent="blocking_agent"),
            ExecutionStep(step_id="next", agent="next_agent", depends_on=["blocking"]),
        ],
    )
    task = asyncio.create_task(Orchestrator(registry).execute(plan, {}))
...
@pytest.mark.asyncio
async def test_orchestrator_repairs_human_review_before_publishing() -> None:
    registry = AgentRegistry()
    expert = FeedbackCapturingAgent()
    audit = HumanReviewRepairSequenceAgent(["needs_human_review", "pass"])
    registry.register("expert_agent", expert)
    registry.register("audit_agent", audit)
    plan = ExecutionPlan(
        plan_id="P_HUMAN_REVIEW_REPAIR", task_type="personalized_review_card", steps=[
            ExecutionStep(step_id="expert", agent="expert_agent"),
            ExecutionStep(step_id="audit", agent="audit_agent", depends_on=["expert"]),
        ]
    )
