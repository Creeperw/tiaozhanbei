...
def test_analyzer_keeps_unknown_numeric_time_budget_absent() -> None:
    ...
    assert "time_budget" in result.gap.missing_fields
    assert "time_budget" not in result.gap.blocking_fields
    assert any(
        uncertainty.category == "time_budget" and not uncertainty.blocking
        for uncertainty in result.bundle.uncertainties
    )
