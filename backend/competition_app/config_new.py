...
_MAIN_SETTINGS = Settings.from_env()
...
# --- 端口配置 ---
# FastAPI 后端端口。vLLM 规划/执行模型占用 8000，因此后端改为 7861。
API_PORT = 7861


# --- 模型配置 ---
# 语音模型本阶段不接入，兼容配置也不允许意外加载。
VOICE_MODEL_PATH = ""
VOICE_MODE = "disabled"
...
SQLITE_PATH = os.getenv("SQLITE_PATH", "./health_agent.db")
...
# --- API 模式配置（统一使用主配置的 OpenAI 兼容端点） ---
# 远程 LLM API Key。生产环境建议通过环境变量注入，不要硬编码到仓库。
LLM_API_KEY = _MAIN_SETTINGS.dashscope_api_key or ""
# 远程 LLM OpenAI 兼容服务地址。
LLM_API_BASE_URL = _MAIN_SETTINGS.chat_base_url
# 远程 LLM 模型名。
LLM_API_MODEL = _MAIN_SETTINGS.chat_model

# --- 本地 vLLM OpenAI 兼容接口配置（LLM_MODE=local 时启用） ---
# Planner/Executor 模型的 OpenAI 兼容服务地址。
PLANNER_EXECUTOR_BASE_URL = _MAIN_SETTINGS.chat_base_url
# Planner/Executor 模型服务名。
PLANNER_EXECUTOR_MODEL = _MAIN_SETTINGS.chat_model
# Manager/Reviewer/Compression 模型的 OpenAI 兼容服务地址。
MANAGER_REVIEWER_BASE_URL = _MAIN_SETTINGS.chat_base_url
...
# 上传文件元数据 JSON 文件路径。
METADATA_FILE = "./APP/backend/file_metadata.json"
...
# markitdown 提取产物预留目录。
MARKITDOWN_OUTPUT_DIR = os.getenv("MARKITDOWN_OUTPUT_DIR", "./APP/backend/markitdown_output")
...
class EmbeddingConfig:
    ...
    EMBEDDING_MODEL_ID = _MAIN_SETTINGS.embedding_model
    ...
    # 向量库根目录。
    BASE_DIR = "./APP/backend/vdb_store"

    # 知识库源文件目录。
    DATA_SOURCE_PATH = "./APP/backend/data"  # 数据目录
    ...
    USER_DATA_ROOT = "./APP/backend/user_knowledge/data"

    # 用户个人知识库向量库根目录，内部按 user_id 分目录隔离。
    USER_INDEX_ROOT = "./APP/backend/user_knowledge/indexes"
