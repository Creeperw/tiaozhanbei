...
class StubChatModel:
    async def complete_text(
        ...
        if role == "evolution_agent":
            signature = business_payload.get("failure_signature") or {}
            text = (
                "该失败模式已形成可追溯聚类，主要表现为"
                f"{signature.get('issue_type', '未分类问题')}，责任节点为"
                f"{signature.get('owner_step_id', 'unknown')}。建议只在该节点应用"
                "白名单预防策略，并通过历史来源覆盖率、首次审核通过率、返修次数"
                "和异常率进行回放观察；当前证据只支持相关性判断，不宣称因果效果。"
            )
            if on_delta:
                on_delta(text)
            return text
        ...
        if role == "diagnosis_agent" and isinstance(result, dict):
            if "learner_evidence" in business_payload or "query_kind" in business_payload:
                # learner_data_query 走纯文本直出：基于只读证据生成自然语言回答。
                # 离线 stub 直接返回 evidence 状态描述，让上层 fallback 逻辑可测。
                evidence = business_payload.get("learner_evidence") or {}
                status = str(evidence.get("evidence_status") or "observed")
                if status == "no_verified_records":
                    text = (
                        f"近{business_payload.get('window_days', 7)}天还没有查到"
                        "可确认的学习完成记录。推荐过、打开过或仅生成过的资源"
                        "不会被算作已经学习。"
                    )
                elif status == "unavailable":
                    text = "我暂时无法读取你的学习记录，请稍后再试。"
                else:
                    text = "已根据系统只读学习记录生成回答。"
            else:
                text = self._diagnosis_plan_document(result)
        else:
            body = (
                result.get("blueprint_document")
                or result.get("assembly_document")
                or result.get("explanation_content")
    ...
    async def complete_json(
        ...
        if role == "planner_agent":
            request_text = str(business_payload.get("user_request", ""))
            prompt_skill_id = str(payload.get("prompt_skill_id") or "")
            ...
            review_task_adjustment = any(
                marker in normalized_request
                for marker in (
                    "少安排", "减少复习", "复习任务减半", "多安排",
                    "增加复习", "取消复习任务", "推迟复习", "复习任务推到",
                )
            )
            selected_task_type = (
                "review_task_adjustment"
                if review_task_adjustment
            ...
            if prompt_skill_id == "planner.route_request":
                source_quote = request_text.strip()[:160] or "（空消息）"
                return self._emit(
                    {
                        "task_type": selected_task_type,
                        "task_source_quote": source_quote,
                        "reason": "根据当前用户消息判断本轮最终交付物。",
                    },
                    on_delta,
                )
            frozen_task_type = str(
                business_payload.get("frozen_task_type") or selected_task_type
            )
            selected_task_type = frozen_task_type
            ...
            plan_action = (
                ...
                if is_plan and effective_scope in {None, "unspecified"}
            ...
            # A structured scope hint supplied by the resume workflow is a
            # trusted interpretation of the user's clarification answer.  It
            # must not retain the earlier ambiguous ``clarify`` action merely
            # because other plan layers already exist.
            has_structured_scope_hint = (
                "plan_scope_hint" in business_payload
                and plan_scope_hint in {"long_term", "short_term", "daily_task"}
            )
            if (
                is_plan
                and has_structured_scope_hint
                and emitted_scope == plan_scope_hint
                and plan_action == "clarify"
            ):
                plan_action = (
                    "reuse"
            ...
            # Emit only fields owned by the frozen branch contract.  Backend
            # dependency completion and audit policy are intentionally not
            # model output in production, so the offline fixture must not
            # pretend they are provider-controlled fields.
            if selected_task_type == "casual_conversation":
                return self._emit(
                    {
                        "task_type": selected_task_type,
                        "casual_response": (
                            ...
                            if memory_record_request
                                ...
                                "我能理解你明天要考试时的焦虑，紧张并不代表你准备得不好。"
                                "先用10分钟做一次低负担回顾：只看最容易混淆的3个要点，"
                                "然后停下来喝水、整理考试用品并尽量按时休息。"
                            )
                            if emotional_support_request
                            else "你好！我是时珍智训智能助教。你想先聊聊当前学习情况，还是直接开始一项学习任务？"
                        ),
                        "emotional_support_request": emotional_support_request,
                        "requires_memory_governance": memory_record_request,
                        "routing_reason": common_reason,
                    },
                    on_delta,
                )
            if selected_task_type == "general_learning_support":
                return self._emit(
                    {
                        "task_type": selected_task_type,
            ...
            if selected_task_type == "knowledge_explanation":
                return self._emit(
                    {
                        "task_type": selected_task_type,
            ...
            if selected_task_type == "learner_data_query":
                return self._emit(
                    {
                        "task_type": selected_task_type,
                        "query_kind": learner_query_kind or "progress_summary",
                        "routing_reason": common_reason,
                    },
                    on_delta,
                )
            if selected_task_type == "learning_plan":
                result = {
                    ...
                    "requires_knowledge_support": plan_needs_knowledge,
                    "routing_reason": common_reason,
                }
                return self._emit(result, on_delta)
            if selected_task_type == "personalized_review_card":
                return self._emit(
                    {
                        "task_type": selected_task_type,
            ...
            if selected_task_type == "paper_generation":
                return self._emit(
                    {
                        "task_type": selected_task_type,
                        "routing_reason": common_reason,
            ...
            if selected_task_type == "review_task_adjustment":
                adjustment = (
                    "reduce_capacity"
                    if any(marker in normalized_request for marker in ("少安排", "减少复习", "减半"))
                    else "increase_capacity"
                    if any(marker in normalized_request for marker in ("多安排", "增加复习"))
                    else "cancel_tasks"
                    if "取消复习" in normalized_request
                    else "snooze_tasks"
                )
                return self._emit(
                    {
                        ...
                        "review_adjustment_source_quote": request_text.strip()[:120],
                        "routing_reason": common_reason,
                    },
                    on_delta,
                )
        if role == "knowledge_base_agent":
            ...
            return self._emit({
                ...
                "uncertainty": [],
                "learning_focus_status": "not_requested",
                "learning_focus_items": [],
            }, on_delta)
        if role == "evolution_rule_compiler":
            source = business_payload.get("failure_signature") or {}
            allowed = business_payload.get("allowed_template_ids") or []
            issue_type = str(source.get("issue_type") or "")
            template = (
                "require_audit_owner_from_dag"
                if "owner" in issue_type or source.get("target_agent") == "audit_agent"
                else "require_summary_ids_exist_in_pack"
                if "summary" in issue_type
                else "require_evidence_ids_from_current_pack"
            )
            if template not in allowed and allowed:
                template = str(allowed[0])
            return {
                "signature_id": source.get("signature_id"),
                "target_agent": source.get("target_agent"),
                "target_step_id": source.get("owner_step_id"),
                "task_type": source.get("task_type"),
                "intervention_type": "prevention",
                "template_id": template,
                "issue_type": source.get("issue_type"),
                "field_path": source.get("field_path") or "",
                "source_case_ids": list(source.get("source_case_ids") or [])[:20],
            }
        ...
        if role == "expert_agent":
            ...
            if phase == "paper_candidate_selection":
                selected = []
                seen_numbers: set[int] = set()
                for unit in business_payload.get("candidate_pool", []):
                    required = int(unit.get("required_question_count", 1))
                    selected_for_unit = 0
                    for item in unit.get("candidates", []):
                        candidate_no = int(item.get("candidate_no", 0) or 0)
                        if (
                            candidate_no <= 0
                            or candidate_no in seen_numbers
                            or not item.get("selectable", False)
                        ):
                            continue
                        seen_numbers.add(candidate_no)
                        selected_for_unit += 1
                        selected.append(
                            {
                                "candidate_no": candidate_no,
                                "rationale": "符合当前蓝图单元且来自系统候选目录。",
                            }
                        )
            ...
            if phase == "paper_gap_generation":
                ...
                return self._emit(
                    {
                        "generation_summary": (
                            f"按系统确认的缺口生成{count}道{question_type}。"
                        ),
                        "generated_items": [
                            {
                                "question_type": question_type,
                                "stem": (
                                    f"{business_payload.get('knowledge_module', '当前主题')}"
                                    f"补充练习题{index + 1}"
                                ),
                                "options": (
                                    ["A. 符合教学结论", "B. 不符合教学结论"]
                                ...
                                "analysis": (
                                    "用于补足用户明确题量，正式发布前由系统审核完整性。"
                                ),
                                "rationale": "补足系统确认的蓝图题量缺口。",
                                "evidence_nos": [],
                            }
            ...
            if phase in {"knowledge_explanation", "general_learning_support"}:
                ...
                return self._emit({
                    ...
                    "explanation_content": (
                        f"## 先理解：{topic}为什么值得学\n\n"
                        "依据教材证据说明其定义、范围和主要表现，先给核心结论。\n\n"
                        "## 核心内容\n\n"
                        "- **定义**：依据教材证据说明其定义、范围和主要表现。\n"
                        "- **关键机制或辨析**：结合教材梳理病因病机、证候关系及容易混淆的边界。\n"
                        "- **学习者易错点**：教学知识用于自我判断时以教材为准，注意区分相近概念。\n\n"
                        "> 提示：先记核心定义，再理解机制和辨析要点。\n\n"
                        "## 思考一下\n\n"
                        "请先用自己的话概括核心概念，并说明它与相近概念的主要区别。"
                    ),
                    "uncertainty": [],
        ...
        if role == "audit_agent":
            if business_payload.get("exam_paper") is not None:
                ...
                # explicit empty list. Generic "verified" prose belongs in the
                # report and must not be compiled into a false repair issue.
                return self._emit(
                    {
                        "decision": "pass",
                        "findings": [],
        ...
        if role == "audit_findings_compiler":
            ...
            def issue_type(text: str) -> str:
                ...
                if any(word in text for word in ("事实错误", "判定错误", "判断错误", "答案错误", "概念错误", "判错", "选错", "写错", "错误结论", "知识点错误", "说法错误", "不准确", "以偏概全", "因果颠倒")):
