...
from competition_app.services.retrieval_fusion import FusionResult, reciprocal_rank_fusion
        ...
        *,
        ...
        for kp_id in resolved_kp_ids:
            for question_id in self.repository.question_ids_for_kp(kp_id):
                ...
                entry["bridge"] = 1.0
        bridge_hits = sorted((question_id, 1.0) for question_id in hits)
        ...
        if candidate_ids:
            ...
            for question_id, score in zip(candidate_ids, scores):
                if score > 0:
                    ...
                    entry["bm25"] = score / maximum
                    bm25_hits.append((question_id, float(entry["bm25"])))
        ...
        fusion = reciprocal_rank_fusion(
            {"bridge": bridge_hits, "bm25": bm25_hits, "vector": vector_hits}
        )
        items = [
            self._detail(question_id, entry, fusion[question_id])
            for question_id, entry in hits.items()
        ]
        ...
        items = reserve_question_details(items, limit=limit)
    ...
    def _detail(
        ...
        return QuestionDetail(
            ...
            retrieval=QuestionRetrievalMetadata(
                channels=channels,
                channel_scores=fusion.channel_scores,
                channel_ranks=fusion.channel_ranks,
                fusion_score=fusion.score,
                fusion_strategy="rrf_v1",
                legacy_max_score=fusion.legacy_max_score,
            ),
        )
