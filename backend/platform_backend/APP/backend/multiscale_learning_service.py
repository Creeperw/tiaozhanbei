...
def _evaluate_hard_constraints(
    *,
    ...
    # Due review is a constraint on the *selected daily set*, not on every
    # individual candidate.  Blocking each new-learning candidate whenever a
    # review is due makes a valid mixed schedule impossible.  The downstream
    # deterministic daily-task scheduler reserves the review quota and records
    # any review that has to be deferred.
    due_ok = True
    safe_under_low_data = (
    ...
    values = {
        ...
        "due_review_priority": (
            due_ok,
            (
                "due_review_candidate"
                if covers_due or descriptor["recommended_action"] == "review"
                else (
                    "pool_level_due_review_constraint_pending"
                    if scope == "daily_task" and due_ids
                    else "no_due_review_constraint"
                )
            ),
...
def build_path_candidates(
    ...
    *,
    ...
    # The formal textbook contract stores prerequisite rules at route level and
    # points them to stages with ``before_stage_id``.  Do not read only
    # ``stage.prerequisites`` here: production stages deliberately omit it.
    prerequisite_route = (
        textbook
        if textbook
        else {
            "route_id": route.get("route_id"),
            "stages": route.get("phases") or [],
            "prerequisites": route.get("prerequisites") or [],
        }
    )
    prerequisite_turn = plan_context.get("prerequisite_turn")
    prerequisite_turn = (
        prerequisite_turn if isinstance(prerequisite_turn, dict) else {}
    )
    persisted_completed = profile.get("completed_courses")
    persisted_completed = (
        persisted_completed if isinstance(persisted_completed, list) else []
    )
    prerequisite_snapshot = resolve_prerequisite_evidence(
        prerequisite_route,
        persisted_completed_courses=persisted_completed,
        recent_messages=(
            prerequisite_turn.get("recent_messages")
            if isinstance(prerequisite_turn.get("recent_messages"), list)
            else []
        ),
        current_user_request=str(
            prerequisite_turn.get("current_user_request") or ""
        ),
        persisted_source_refs=profile_refs,
    )
    satisfied_prerequisites = {
        normalize_course_name(item)
        for item in prerequisite_snapshot.satisfied_courses
    }
    ...
    for descriptor in descriptors:
        descriptor_stage = (
            ...
            if isinstance(descriptor.get("stage"), dict)
            else {}
        )
        descriptor_stage_id = str(
            descriptor_stage.get("stage_id")
            or descriptor_stage.get("phase_id")
            or ""
        ).strip()
        ...
        missing_prerequisites = [
            course
            for course in required_prerequisites
            if normalize_course_name(course) not in satisfied_prerequisites
        ]
        prerequisite_evidence_refs = (
            prerequisite_snapshot.source_refs if required_prerequisites else []
        )
    ...
    return {
        ...
        "prerequisite_evidence": prerequisite_snapshot.model_dump(mode="json"),
        "items": results,
        "counts": {
