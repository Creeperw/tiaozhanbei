...
from APP.backend.section_exam_service import (
    SectionExamError,
    submit_section_exam_answer as submit_section_exam_answer_service,
)
...
class SectionExamAnswerRequest(BaseModel):
    """One electronic-textbook worksheet answer.

    ``request_id`` is generated by the client when the answer changes, so
    re-revealing an unchanged answer replays the stored verdict instead of
    writing a second attempt.
    """
...
def get_questions_by_kp_ids(
    ...
    results = hybrid_rank_questions(candidates, kp_ids=ids, limit=limit)
    return {"items": results, "total": len(results)}

@router.post("/section-exam/answers")
@stable_practice_router.post("/section-exam/answers")
def submit_section_exam_answer(
    request: SectionExamAnswerRequest,
    current_user: UserModel = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    """Grade and record one electronic-textbook section worksheet answer."""
    try:
        return submit_section_exam_answer_service(
            db,
            current_user.id,
    ...
    except SectionExamError as exc:
