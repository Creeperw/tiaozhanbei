...
LEARNING_TARGET_CHANGED_HEADER = "X-Competition-Learning-Target-Changed"
...
class LearnerProfileUpdate(BaseModel):
    ...
    resource_preference: Optional[list[str]] = None
...
def enroll_learning_targets(
    ...
    response.headers[LEARNING_TARGET_CHANGED_HEADER] = "1"
    items = [serialize_learning_target(item) for item in list_learning_targets(db, current_user.id)]
...
def switch_current_learning_target(
    ...
    return update_learning_target(body, response, current_user, db)
...
def get_profile(current_user: UserModel = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    payload = build_learner_profile_payload(profile)
    return {storage: payload[field] for storage, field in REVERSE_FIELD_MAP.items()}

@router.put("/profile")
def update_profile(body: ProfileUpdate, response: Response, current_user: UserModel = Depends(get_current_user), db: Session = Depends(get_db)):
    profile = get_or_create_profile(db, current_user.id)
    apply_learner_profile_update(profile, {
        REVERSE_FIELD_MAP[key]: value for key, value in body.model_dump(exclude_unset=True).items()
    })
    db.commit()
...
def update_learner_profile(body: LearnerProfileUpdate, response: Response, current_user: UserModel = Depends(get_current_user), db: Session = Depends(get_db)):
    profile = get_or_create_profile(db, current_user.id)
