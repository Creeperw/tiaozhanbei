...
def next_practice_question(
    ...
    for question in db.query(QuestionBankItem).filter(
        ...
        candidates.append({
            ...
            "kp_ids": question_kp_ids,
            "question_type": question.question_type,
            "quality_score": float(question.quality_score or 0),
            "_question": question,
        })
    ...
    ranked_candidates = hybrid_rank_questions(
        candidates,
    ...
    ranked_candidates.sort(key=lambda item: (
        item["question_id"] in active_claims,
        item["question_id"] in attempted_question_ids,
    ...
    if exclude_question_id:
        ...
        ranked_candidates = [
            ...
            if item["question_id"] != exclude_question_id
        ]
    ...
    question = selected["_question"]
    question_kp_ids = selected["kp_ids"]
    effective_difficulty = question.difficulty
    if effective_difficulty is None and question.question_id in user_difficulty_tags:
        effective_difficulty = user_difficulty_tags[question.question_id]
    request_id = str(uuid.uuid4())
