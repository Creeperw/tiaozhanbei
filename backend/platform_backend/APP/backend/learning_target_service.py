...
def list_learning_targets(db: Session, user_id: int) -> list[UserLearningTarget]:
    """Return one durable enrollment per exam, current target first.

    Older builds created a new row every time an archived target was selected
    again.  Keep the newest row for each exam in the compatibility projection;
    new writes reactivate that row instead of manufacturing another workspace.
    """

    rows = (
        db.query(UserLearningTarget)
        .filter(UserLearningTarget.user_id == user_id)
        .order_by(
            UserLearningTarget.is_active.desc(),
            UserLearningTarget.updated_at.desc(),
            UserLearningTarget.id.desc(),
        )
        .all()
    )
    unique: list[UserLearningTarget] = []
    seen: set[str] = set()
    for row in rows:
        if row.exam_track_id in seen:
            continue
        seen.add(row.exam_track_id)
        unique.append(row)
    return unique
...
def set_active_learning_target(
    ...
    *,
    ...
    existing = (
        db.query(UserLearningTarget)
        .filter(
            UserLearningTarget.user_id == user_id,
            UserLearningTarget.exam_track_id == exam_track_id,
        )
        .order_by(UserLearningTarget.updated_at.desc(), UserLearningTarget.id.desc())
        .first()
    )
    if active is not None:
        ...
        # Switching the current exam does not remove the enrollment.  The row
        # remains the durable workspace head and can be reactivated later.
        active.archived_at = None
        active.updated_at = now

    # Flush the released active_slot before reusing it on another row.  This is
    # required by the one-current-target unique index on both MySQL and SQLite.
    db.flush()

    if existing is not None and existing is not active:
        existing.target_type = target_type
        existing.exam_name_snapshot = str(
            track.get("qualification_name")
            or track.get("title_normalized")
            or track.get("title")
            or exam_track_id
        )
        existing.syllabus_version = str(
            track.get("schema_version") or track.get("year") or ""
        )
        existing.exam_date = _as_datetime(exam_date)
...
def serialize_learning_target(target: UserLearningTarget | None) -> dict | None:
    ...
    return {
        "id": target.id,
        # Stable server-owned exam workspace identifier.  It is intentionally
        # distinct from the public catalog target_id used by the frontend.
        "scope_id": f"EXS_{target.id}",
