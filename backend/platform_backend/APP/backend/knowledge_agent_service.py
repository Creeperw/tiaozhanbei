...
def _question_evidence(db: Session, kp_ids: list[str], limit: int = 5) -> list[dict[str, Any]]:
    ...
    try:
        candidates.extend(atlas_service.questions_for_kps(sorted(targets), limit=max(1, int(limit)) * 4))
    except (AtlasUnavailableError, OSError, RuntimeError):
        pass
    if hasattr(db, "query"):
    ...
    unique_candidates = {
        str(candidate.get("question_id")): candidate
        for candidate in candidates
        if candidate.get("question_id")
    }
    ranked = hybrid_rank_questions(list(unique_candidates.values()), kp_ids=targets, limit=limit)
    return [
        {
            "question_id": row["question_id"],
            "stem": row["stem"],
            ...
            "analysis": row.get("analysis") or row.get("explanation", ""),
...
def list_questions(db: Session, kp_ids: list[str] | None = None, limit: int = 20) -> list[dict[str, Any]]:
    ...
    return [
        ...
        for row in hybrid_rank_questions(candidates, kp_ids=targets, limit=limit)
    ]
