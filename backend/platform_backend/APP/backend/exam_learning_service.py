...
def _workspace_progress_rows(db: Session, user_id: int, kp_ids) -> tuple[list, list, bool]:
    """Read the active exam projection, falling back only for legacy users."""

    target = get_active_learning_target(db, user_id)
    if target is not None:
        rows = db.query(LearnerExamProgressState).filter(
            LearnerExamProgressState.learner_id == user_id,
            LearnerExamProgressState.exam_track_id == target.exam_track_id,
            LearnerExamProgressState.kp_id.in_(kp_ids),
        ).all()
    ...
    return mastery_rows, review_rows, False
...
def get_learner_knowledge_point_state(
    ...
    *,
    ...
    target = get_active_learning_target(db, user_id)
    if target is not None:
        mastery = db.query(LearnerExamProgressState).filter_by(
            learner_id=user_id,
            exam_track_id=target.exam_track_id,
            kp_id=kp_id,
        ).one_or_none()
        review = mastery
        mistake_count = int(mastery.wrong_count or 0) if mastery is not None else 0
    else:
...
def get_node_learner_summary(
    ...
    *,
    ...
    mastery_rows, review_rows, _ = _workspace_progress_rows(db, user_id, kp_ids)
    scores = [
...
def get_visible_node_learner_states(
    ...
    *,
    ...
    review_by_kp = {}
    if all_kp_ids:
        mastery_rows, review_rows, _ = _workspace_progress_rows(
            db, user_id, all_kp_ids
        )
