...
class ExternalIdentityLink(Base):
    """Stable mapping from the host application's user id to this domain's integer id."""
    ...
    __table_args__ = (
        UniqueConstraint(
            "provider", "external_user_id", name="uq_external_identity_provider_user"
        ),
    )
...
class LearnerExamProgressState(Base):
    """Independent derived learning state for one learner/exam/KP workspace.

    The authoritative v2 tables above are retained for backward compatibility
    with historical training records.  New multi-certificate flows read this
    workspace projection so the same KP can have different mastery and review
    state under different examinations.
    """

    __tablename__ = "learner_exam_progress_states"
    id = Column(Integer, primary_key=True, index=True)
    progress_state_id = Column(String(120), unique=True, nullable=False, index=True)
    learner_id = Column(Integer, ForeignKey("users.id"), nullable=False, index=True)
    exam_track_id = Column(String(120), nullable=False, index=True)
    kp_id = Column(String(120), nullable=False, index=True)
    mastery_score = Column(Float, default=0.0)
    ...
    __table_args__ = (
        UniqueConstraint(
            ...
            "exam_track_id",
            "kp_id",
            name="uq_exam_progress_learner_exam_kp",
        ),
        Index(
            "ix_exam_progress_learner_exam_updated",
            "learner_id",
            "exam_track_id",
            "updated_at",
        ),
...
class LearnerExamAttemptMembership(Base):
    """Tags an immutable raw attempt with the exam active at writeback time."""

    __tablename__ = "learner_exam_attempt_memberships"
    id = Column(Integer, primary_key=True, index=True)
    learner_id = Column(Integer, ForeignKey("users.id"), nullable=False, index=True)
    exam_track_id = Column(String(120), nullable=False, index=True)
    attempt_id = Column(
        String(120), ForeignKey("learning_attempts.attempt_id"), nullable=False, index=True
    )
    created_at = Column(DateTime, default=utc_now)
    __table_args__ = (
        UniqueConstraint(
            ...
            "exam_track_id",
            "attempt_id",
            name="uq_exam_attempt_membership",
        ),
        Index(
            "ix_exam_attempt_membership_scope",
            "learner_id",
            "exam_track_id",
            "created_at",
        ),
...
class PlanReviewRecord(Base):
    ...
    # Execution is a separate lifecycle from generating the proposal.  Keep
    # it durable so a browser refresh cannot make a queued/running operation
    # look like a completed review.
    execution_status = Column(String(30), nullable=False, default="not_started", index=True)
...
class KnowledgePointCanonicalMap(Base):
    """Auditable source-to-canonical identity decision for one knowledge point."""

    __tablename__ = "knowledge_point_canonical_maps"
    id = Column(Integer, primary_key=True, index=True)
    mapping_id = Column(String(120), unique=True, nullable=False, index=True)
    source_kp_id = Column(String(120), unique=True, nullable=False, index=True)
    canonical_kp_id = Column(String(120), nullable=False, index=True)
    decision = Column(String(50), nullable=False, default="equivalent", index=True)
    decision_basis = Column(String(120), nullable=False, default="manual_review")
    confidence = Column(Float, nullable=False, default=1.0)
    status = Column(String(50), nullable=False, default="active", index=True)
    evidence_json = Column(Text, nullable=False, default="{}")
    decided_by = Column(String(120), nullable=False, default="system")
    decided_at = Column(DateTime, nullable=False, default=utc_now)
    created_at = Column(DateTime, nullable=False, default=utc_now)
    updated_at = Column(DateTime, nullable=False, default=utc_now, onupdate=utc_now)
    __table_args__ = (
        Index(
            "ix_kp_canonical_map_target_status",
            "canonical_kp_id",
            "status",
        ),
    )
...
class KnowledgePointCanonicalMigration(Base):
    """Immutable-enough audit record for a reviewed canonical replay batch."""

    __tablename__ = "knowledge_point_canonical_migrations"
    id = Column(Integer, primary_key=True, index=True)
    migration_id = Column(String(120), unique=True, nullable=False, index=True)
    canonical_kp_id = Column(String(120), nullable=False, index=True)
    source_kp_ids_json = Column(Text, nullable=False, default="[]")
    mode = Column(String(30), nullable=False, default="dry_run", index=True)
    status = Column(String(50), nullable=False, default="prepared", index=True)
    plan_hash = Column(String(120), nullable=False, default="event_replay_v1")
    report_json = Column(LARGE_JSON_TEXT, nullable=False, default="{}")
    rollback_json = Column(LARGE_JSON_TEXT, nullable=False, default="{}")
    requested_by = Column(String(120), nullable=False, default="operator")
    created_at = Column(DateTime, nullable=False, default=utc_now)
...
_LEARNING_GOVERNANCE_TABLES = (
    "knowledge_point_canonical_maps",
    "knowledge_point_canonical_migrations",
...
def _ensure_learning_governance_tables(bind):
    ...
    inspector = inspect(bind)
    if "plan_review_records" not in inspector.get_table_names():
        return
    existing_columns = {
        column["name"] for column in inspector.get_columns("plan_review_records")
    }
    additions = {
        "execution_status": "VARCHAR(30) NOT NULL DEFAULT 'not_started'",
        # MySQL rejects DEFAULT expressions on TEXT columns.  The ORM default
        # still supplies the value for new rows; legacy rows are backfilled
        # below and the column remains nullable for cross-version compatibility.
        ...
        for column_name, definition in additions.items():
            if column_name not in existing_columns:
                _add_column_if_missing_after_race(
                    ...
                    "plan_review_records",
                    column_name,
                    f"ALTER TABLE plan_review_records ADD COLUMN {column_name} {definition}",
                )
        if "execution_json" not in existing_columns:
            connection.execute(
                text(
                    "UPDATE plan_review_records SET execution_json = '{}' "
                    "WHERE execution_json IS NULL"
                )
            )


def _ensure_knowledge_identity_tables(bind):
    """Create additive, audit-only canonical identity tables."""

    Base.metadata.create_all(
        bind=bind,
        tables=[
            Base.metadata.tables[table_name]
            for table_name in _KNOWLEDGE_IDENTITY_TABLES
        ],
    )
...
def ensure_runtime_schema_for(bind, checkpoint=lambda stage: None):
    ...
    if bind.dialect.name == "mysql":
        ...
        try:
            inspector = inspect(bind)
            # The original phase-three migrator assumed an already populated
            # deployment. A newly created integration database needs the full
