...
_asset_root_raw = os.getenv("SHIZHEN_ASSET_ROOT", "").strip()
_ASSET_ROOT = Path(_asset_root_raw).expanduser() if _asset_root_raw else _PROJECT_ROOT / "assets"
if not _ASSET_ROOT.is_absolute():
    _ASSET_ROOT = (_PROJECT_ROOT / _ASSET_ROOT).resolve()
_CHAPTER_RELEASE = os.getenv("SHIZHEN_ATLAS_CHAPTER_RELEASE", "2026-07-22").strip()
_CANONICAL_CHAPTER_ROOT = (
    _ASSET_ROOT / "knowledge-atlas" / "chapters" / "releases" / _CHAPTER_RELEASE
)
_LEGACY_CHAPTER_ROOT = (
    _PROJECT_ROOT
    / "backend"
    / "competition"
...
_REPOSITORY_CHAPTER_ROOT = (
    _CANONICAL_CHAPTER_ROOT
    if _CANONICAL_CHAPTER_ROOT.exists() or not _LEGACY_CHAPTER_ROOT.exists()
    else _LEGACY_CHAPTER_ROOT
)
...
_CONFIGURED_CHAPTER_ROOT = os.getenv("KNOWLEDGE_ATLAS_CHAPTER_ROOT", "").strip()
...
class KnowledgeAtlasStore:
    ...
    def chapter_root(self) -> Path:
        ...
        # The main application projects its resolved asset path only while this
        # compatibility backend is imported. Capture that value so requests do
        # not fall back to a path derived from the backend's former dated folder.
        configured = _CONFIGURED_CHAPTER_ROOT or os.getenv(
            "KNOWLEDGE_ATLAS_CHAPTER_ROOT", ""
        ).strip()
        if configured:
    ...
    def questions_for_kps(
        ...
        *,
        ...
        self.ensure_questions()
        normalized_kp_ids = [str(value) for value in kp_ids if str(value)]
        selected: dict[str, dict[str, Any]] = {}
        for kp_id in normalized_kp_ids:
            for question in self.questions_by_kp.get(kp_id, ()):
                selected.setdefault(question["question_id"], question)
        return hybrid_rank_questions(
            [
                {
                    **question,
    ...
    def search_questions(
        ...
        *,
        ...
        return hybrid_rank_questions(
            [
                {**question, "channels": ["atlas_question_bank"]}
