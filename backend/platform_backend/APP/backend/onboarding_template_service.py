...
PREVIOUS_GROUP_TEMPLATES: list[dict[str, Any]] = [
    {
...
LEGACY_GROUP_TEMPLATES = [*PREVIOUS_GROUP_TEMPLATES, *LEGACY_GROUP_TEMPLATES]
...
GROUP_TEMPLATES: list[dict[str, Any]] = [
    {
        ...
        "tags": [readiness],
        "examples": [],
        "baseline_background": {"education_major": background, "foundation_level": readiness},
        # Readiness does not imply an individual's time budget or preference.
        "default_profile": {
            "learning_goal": "", "daily_available_minutes": 30,
            "preferred_time_slot": "", "resource_preference": [],
            "learning_mode": "", "current_difficulties": [],
        },
    }
    for key, title, background, readiness in (
...
QUESTIONS: list[dict[str, Any]] = [
    {
        ...
        "help_text": "低、中、高仅描述专业背景与前置知识就绪度，不代表实时掌握度。",
        "default_by_group": {},
    },
    {
    ...
    },
    {
        ...
        "type": "single_choice",
        "required": False,
        "options": ["早晨", "午休", "晚间或碎片化时间", "晚间固定学习时段", "周末", "碎片化不固定", {"value": "", "label": "暂未选择"}],
...
def apply_onboarding_defaults(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if group.get("baseline_background"):
        # Explicit fixed baseline selection, never open-text classification.
        background.update(deepcopy(group["baseline_background"]))
        background["major_or_role"] = background["education_major"]
        background["tcm_foundation"] = background["foundation_level"]
        normalized["education_major"] = background["education_major"]
        normalized["major_or_role"] = background["education_major"]
        normalized["tcm_foundation"] = background["foundation_level"]
        normalized["learning_background"] = background["foundation_level"]
    ...
    if "resource_preference" not in preferences:
        preferences["resource_preference"] = deepcopy(
            normalized.get("resource_preference", _question_default("resource_preference", group["key"]))
        )
    if preferences["resource_preference"] in (None, ""):
        preferences["resource_preference"] = []
    if "resource_preference" in normalized:
        normalized["resource_preference"] = deepcopy(preferences["resource_preference"])
