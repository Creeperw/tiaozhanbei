...
def normalize_onboarding_answers(survey_answers: dict[str, Any], learner_group: str = "") -> dict[str, Any]:
    ...
    normalized = {
        ...
        "resource_preference": _listify(preferences.get("resource_preference", survey_answers.get("resource_preference"))),
        ...
        "custom_requirements": _text(
            survey_answers.get("custom_requirements")
            or special.get("custom")
            or special.get("custom_requirements")
        ),
    }
...
def submit_onboarding_survey(
    ...
    if "resource_preferences" in existing_locked_fields:
        from APP.backend.learner_profile_service import read_resource_preferences
        preserved = read_resource_preferences(profile)[0]
        stored_survey["resource_preference"] = preserved
        stored_survey["preferences"] = {**(stored_survey.get("preferences") or {}), "resource_preference": preserved}
        normalized["resource_preference"] = preserved
        l0_baseline["resource_preference"] = preserved
    ...
    profile_update = {
        ...
        "resource_preferences": "、".join(l0_baseline["resource_preference"]),
    ...
    # Default-filled fields must not overwrite existing user locks.
...
def get_onboarding_status(db: Session, user_id: int) -> dict[str, Any]:
    ...
    if payload:
        ...
        current_survey = _json_dict(getattr(profile, "survey_json", None))
        answers = dict(sanitized.get("survey_answers", {}))
        if current_survey:
            answers.update(normalize_onboarding_answers(current_survey, _text(profile.constitution)))
        from APP.backend.learner_profile_service import read_resource_preferences
        answers["resource_preference"] = read_resource_preferences(profile)[0]
        field_sources = dict(sanitized.get("field_sources") or {})
        current_sources = current_survey.get("field_sources")
