...
_configured_package_root = os.getenv("KNOWLEDGE_HANDOFF_ROOT", "").strip()
_configured_asset_root = os.getenv("SHIZHEN_ASSET_ROOT", "").strip()
DEFAULT_PACKAGE_ROOT = Path(
    _configured_package_root
    or str(
        Path(_configured_asset_root or PROJECT_ROOT / "assets")
        / "knowledge"
        / "releases"
        / (os.getenv("SHIZHEN_ASSET_RELEASE", "").strip() or ASSET_VERSION)
    )
).expanduser().resolve()
