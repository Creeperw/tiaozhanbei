...
class QuestionRepository:
    ...
    def select(self, criteria: QuestionSelectionCriteria):
        ...
        try:
            ...
            ranked_items = hybrid_rank_questions(
                [
                    {
                        ...
                        "kp_ids": list(self._kp_ids(item)),
                        "question_type": item.question_type,
                        "_item": item,
                    }
                    for item in items
                ],
                query="、".join(criteria.kp_ids),
                kp_ids=criteria.kp_ids,
                limit=len(items) or 1,
            )
            items = [row["_item"] for row in ranked_items]
