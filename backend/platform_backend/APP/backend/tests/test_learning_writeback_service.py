...
from APP.backend.database import (
    ...
    DailyTaskItemRecord,
    KnowledgeMasteryState, KnowledgePoint, KnowledgePointCanonicalMap,
    LearnerExamAttemptMembership, LearnerExamProgressState,
...
class LearningWritebackServiceTests(unittest.TestCase):
    ...
    def test_active_exam_writeback_updates_independent_workspace_projection(self):
        from APP.backend.learning_target_service import set_active_learning_target
        ...
        self.assertEqual(result.status, "applied")
        progress = self.db.query(LearnerExamProgressState).one()
        self.assertEqual(progress.exam_track_id, "EXAM_2025_TCM_PHYSICIAN")
        self.assertEqual(progress.kp_id, "kp-1")
        self.assertEqual(progress.attempt_count, 1)
        self.assertAlmostEqual(progress.mastery_score, 100.0)
        membership = self.db.query(LearnerExamAttemptMembership).one()
        self.assertEqual(membership.exam_track_id, "EXAM_2025_TCM_PHYSICIAN")
        self.assertEqual(membership.attempt_id, "attempt-1")
    ...
    def test_does_not_invent_attempts_from_legacy_scalar_counters(self):
        ...
        expected_rate = lambda_per_day(0, 0)
        ...
        self.assertAlmostEqual(review.lambda_per_day, lambda_per_day(0, 1))
        self.assertEqual(
            (
                review.recent_five_wrong_count,
                review.consecutive_independent_correct,
                review.consecutive_wrong_count,
                review.review_stage,
            ),
            (0, 1, 0, "4"),
        )

    def seed_sequential_attempt(self, suffix, correct, *, learner_id=1, kp_id="kp-1", decision="pass"):
        attempt_id, item_id = f"attempt-{suffix}", f"item-{suffix}"
        artifact, pack, audit = f"grade-{suffix}", f"pack-{suffix}", f"audit-{suffix}"
        self.db.add(LearningAttemptRecord(attempt_id=attempt_id, learner_id=learner_id, status="submitted"))
        self.db.flush()
        self.db.add(LearningAttemptItemRecord(attempt_item_id=item_id, attempt_id=attempt_id, question_version_id="qv-1", kp_snapshot_json=json.dumps([{"kp_id": kp_id}])))
        self.db.flush()
        self.db.add(EvidencePackRecord(pack_id=pack, user_id=learner_id, resolved_kp_ids_json=json.dumps([kp_id]), payload_json=json.dumps({"attempt_item_id": item_id, "question_version_id": "qv-1"})))
        self.db.add(GradingResultRecord(artifact_id=artifact, attempt_item_id=item_id, version=1, score=100 if correct else 0, max_score=100, is_correct=correct, kp_ids_json=json.dumps([kp_id]), evidence_pack_id=pack, confidence=.9, status="reviewed", payload_json=json.dumps({"question_version_id": "qv-1"})))
        self.db.flush()
        self.db.add(AuditResultRecord(audit_id=audit, source_artifact_id=artifact, source_artifact_version=1, decision=decision, status="completed"))
        self.db.commit()
        return GradingWritebackCommand(item_id, artifact, 1, audit)

    def test_recent_five_wrong_window_matches_audited_replay_and_is_idempotent(self):
        from APP.backend.knowledge_point_history_replay import collect_canonical_replay_events, replay_canonical_events

        outcomes = [False, True, True, True, True, True, False]
        for index, correct in enumerate(outcomes):
            command = self.seed_sequential_attempt(f"seq-{index}", correct)
            first = apply_grading_writeback(self.db, 1, command)
            self.db.commit()
            self.assertEqual(apply_grading_writeback(self.db, 1, command), first)
            review = self.db.query(LearnerKPReviewState).one()
            self.assertEqual(review.recent_five_wrong_count, sum(not value for value in outcomes[max(0, index - 4):index + 1]))
            events = collect_canonical_replay_events(self.db, source_kp_ids=("kp-1",), learner_id=1, admitted_only=True)
            projection = replay_canonical_events(events)[0]
            self.assertEqual(review.recent_five_wrong_count, projection.recent_five_wrong_count)
            self.assertEqual(review.consecutive_independent_correct, projection.consecutive_independent_correct)
            self.assertEqual(review.consecutive_wrong_count, projection.consecutive_wrong_count)
            self.assertAlmostEqual(review.lambda_per_day, projection.lambda_per_day)
        self.assertEqual(self.db.query(LearningWritebackReceipt).count(), len(outcomes))

    def test_recent_wrong_window_excludes_other_users_kps_and_unwritten_grading(self):
        self.db.add(UserModel(id=2, username="other", hashed_password="x"))
        self.db.commit()
        other_user = self.seed_sequential_attempt("other-user", False, learner_id=2)
        apply_grading_writeback(self.db, 2, other_user)
        self.db.commit()
        other_kp = self.seed_sequential_attempt("other-kp", False, kp_id="kp-other")
        apply_grading_writeback(self.db, 1, other_kp)
        self.db.commit()
        self.seed_sequential_attempt("unwritten", False)
        rejected = self.seed_sequential_attempt("rejected", False, decision="reject")
        apply_grading_writeback(self.db, 1, rejected)
        valid = self.seed_sequential_attempt("valid", True)
        apply_grading_writeback(self.db, 1, valid)
        review = self.db.query(LearnerKPReviewState).filter_by(learner_id=1, kp_id="kp-1").one()
        self.assertEqual(review.recent_five_wrong_count, 0)
        self.assertEqual(review.consecutive_independent_correct, 1)

    def test_wrong_attempt_advances_review_counters_for_next_formula(self):
        apply_grading_writeback(self.db, 1, self.seed(correct=False))
    ...
    def test_recent_wrong_window_combines_mapped_source_attempts(self):
        self.db.add(KnowledgePointCanonicalMap(mapping_id="mapped-window", source_kp_id="kp-old", canonical_kp_id="kp-1", decision="equivalent", status="active"))
        self.db.commit()
        old = self.seed_sequential_attempt("old", False, kp_id="kp-old")
        apply_grading_writeback(self.db, 1, old)
        self.db.commit()
        current = self.seed_sequential_attempt("current", True)
        apply_grading_writeback(self.db, 1, current)
        review = self.db.query(LearnerKPReviewState).one()
        self.assertEqual(review.kp_id, "kp-1")
        self.assertEqual(review.recent_five_wrong_count, 1)
        self.assertEqual(review.consecutive_independent_correct, 1)

    def test_recent_wrong_window_excludes_incomplete_daily_task_receipt(self):
        command = self.seed_sequential_attempt("daily", False)
        attempt = self.db.query(LearningAttemptRecord).filter_by(attempt_id="attempt-daily").one()
        attempt.daily_task_item_id = "DAILY_PENDING"
        self.db.commit()
        skipped = apply_grading_writeback(self.db, 1, command)
        self.assertEqual(skipped.mastery_updates, ())
        self.db.commit()
        valid = self.seed_sequential_attempt("after-daily", True)
        apply_grading_writeback(self.db, 1, valid)
        review = self.db.query(LearnerKPReviewState).one()
        self.assertEqual(review.recent_five_wrong_count, 0)
        self.assertEqual(review.consecutive_independent_correct, 1)
    ...
    def test_artifact_versions_have_independent_idempotency_receipts(self):
        ...
        review = self.db.query(LearnerKPReviewState).one()
        self.assertEqual(review.recent_five_wrong_count, 1)
        self.assertEqual(review.consecutive_wrong_count, 1)
    ...
    def test_same_attempt_duplicate_source_ids_update_one_canonical_concept(self):
        self.db.add_all([
            KnowledgePoint(kp_id="kp-canonical", name="四君子汤配伍意义"),
            KnowledgePoint(
                kp_id="kp-duplicate",
                name="四君子汤配伍意义",
            ...
            KnowledgePointCanonicalMap(
                mapping_id="map-duplicate",
                source_kp_id="kp-duplicate",
                canonical_kp_id="kp-canonical",
                decision="equivalent",
                decision_basis="reviewed_exact_equivalence",
                confidence=1.0,
                status="active",
                evidence_json='{"review":"test"}',
        ...
        result = apply_grading_writeback(
            ...
            1,
            self.seed(
                grading_kps=("kp-canonical", "kp-duplicate"),
                snapshot=(
                    {"kp_id": "kp-canonical", "relation_type": "primary", "confidence": 1.0},
                    {"kp_id": "kp-duplicate", "relation_type": "secondary", "confidence": 1.0},
                ),
            ),
        )
        ...
        self.assertEqual(
            ...
            ({"kp_id": "kp-canonical", "mastery_score": 0.0},),
        )
        self.assertEqual(self.db.query(KnowledgeMasteryState).count(), 1)
        self.assertEqual(self.db.query(MasteryHistoryRecord).count(), 1)
        self.assertEqual(self.db.query(LearnerKPReviewState).count(), 1)
        history = self.db.query(MasteryHistoryRecord).one()
        self.assertEqual(history.kp_id, "kp-canonical")
