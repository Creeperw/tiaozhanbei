...
class MultiScaleLearningServiceTests(unittest.TestCase):
    ...
    def test_route_level_prerequisite_blocks_formal_stage_without_stage_field(self) -> None:
        context = approved_plan_context()
        route = context["long_term_plan"]["planning_route"]
        route["textbook_route"] = {
        ...
        self.assertFalse(result["passed"])
        self.assertIn("中医诊断学", result["reason"])
        self.assertEqual(
            candidates["prerequisite_evidence"]["unknown_courses"],
            ["中医诊断学"],
        )
    ...
    def test_current_turn_forgetting_overrides_persisted_completion(self) -> None:
        ...
        route["textbook_route"] = {
            ...
            "route": {
                ...
                "stages": [
                    {"stage_id": "stage-1", "order": 1, "name": "基础", "books": ["《中医学基础》"]},
                    {"stage_id": "stage-2", "order": 2, "name": "方剂", "books": ["《方剂学》"]},
                ],
            },
        }
        context["long_term_plan"]["textbook_selection"] = {
        ...
        context["prerequisite_turn"] = {
            "current_user_request": "中医诊断学以前学完过，但现在基本忘了。",
            "recent_messages": [],
        }
        self.db.query(database.LearningUserProfile).filter(
            database.LearningUserProfile.user_id == 1
        ).update({"completed_courses_json": '["中医诊断学"]'})
        self.db.commit()
        state = build_multiscale_state(self.db, 1, plan_context=context)
        ...
        self.assertFalse(result["passed"])
        self.assertEqual(
            candidates["prerequisite_evidence"]["unmet_courses"],
            ["中医诊断学"],
        )
        self.assertIn("current_user_request", result["source_refs"])
