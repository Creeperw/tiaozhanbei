        ...
        try:
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                try:
                    ...
                    # The seed contains observed question attempts but no
                    # published atomic daily-task denominator. Missing task
                    # evidence is neutral rather than fabricated as failure.
                    self.assertFalse(
                        behavior_window["metric_availability"]["task_completion_rate"]
                    )
                    self.assertEqual(behavior_window["task_completion_rate"], 1.0)
                    self.assertEqual(
                        behavior_window["sample_counts"][
                            "daily_task_items_current_window"
                        ],
                        0,
                    )
