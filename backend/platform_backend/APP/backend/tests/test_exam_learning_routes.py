...
class ExamLearningRoutesTests(unittest.TestCase):
    ...
    def test_multiple_exam_enrollments_are_reactivated_instead_of_recreated(self):
        enrolled = self.client.post(
            "/personalization/learning-targets",
            json={
                "exam_track_ids": [
        ...
        self.assertEqual(len(enrolled.json()["items"]), 2)
        original_ids = {
            item["exam_track_id"]: item["scope_id"]
            for item in enrolled.json()["items"]
        }
        ...
        self.assertEqual(
            switched.json()["target"]["exam_track_id"],
            "EXAM_2025_INTEGRATED_ASSISTANT",
        )

        listed = self.client.get("/personalization/learning-targets")
    ...
    def test_same_knowledge_point_keeps_independent_progress_per_exam(self):
        ...
        with self.Session() as db:
            ...
            db.add_all([
                database.LearnerExamProgressState(
                    progress_state_id="progress-tcm",
                    learner_id=1,
                    exam_track_id="EXAM_2025_TCM_PHYSICIAN",
                    kp_id="kp-accepted",
                    mastery_score=91.0,
                ...
                database.LearnerExamProgressState(
                    progress_state_id="progress-integrated",
                    learner_id=1,
        ...
        self.assertEqual(tcm.status_code, 200)
        self.assertEqual(tcm.json()["mastery_score"], 91.0)
        self.assertEqual(tcm.json()["attempt_count"], 8)
        ...
        self.assertEqual(switched.status_code, 200)
        integrated = self.client.get(
            "/exam-learning/knowledge-points/kp-accepted/learner-state"
        )
        self.assertEqual(integrated.status_code, 200)
        self.assertEqual(integrated.json()["mastery_score"], 27.0)
        self.assertEqual(integrated.json()["attempt_count"], 2)
