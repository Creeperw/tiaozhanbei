...
class TrainingRoutesBehaviorTests(unittest.TestCase):
    ...
    def test_onboarding_group_templates_include_required_groups_and_questions(self):
        ...
        self.assertEqual(group_keys, ["low", "medium", "high"])
        ...
        cross = body["groups"][0]
        self.assertIn("零基础非医学专业", cross["tags"])
        self.assertIn("default_profile", cross)

    def test_baseline_submit_and_preference_edit_share_current_survey(self):
        from APP.backend.learning_governance_service import _resource_preference_details
        response = self.client.post('/training/onboarding/survey', json={
            'learner_group': 'medium', 'preferences': {'resource_preference': ['案例训练', '视频']},
        })
        self.assertEqual(response.status_code, 200)
        detail = self.client.get('/personalization/learner-profile').json()
        self.assertEqual(detail['education_major'], '医学相关专业，接触过中医基础课程')
        self.assertEqual(detail['resource_preferences'], '案例训练、视频')
        updated = self.client.put('/personalization/learner-profile', json={'resource_preference': ['讲义讲解']})
        self.assertEqual(updated.status_code, 200)
        self.assertEqual(updated.headers.get('X-Competition-Learner-Profile-Changed'), '1')
        current_status = self.client.get('/training/onboarding/status').json()
        self.assertEqual(current_status['survey_answers']['resource_preference'], ['讲义讲解'])
        self.assertEqual(current_status['field_sources']['preferences.resource_preference'], 'user_confirmed')
        with self.Session() as db:
            profile = db.query(database.UserProfile).filter_by(user_id=1).one()
            self.assertEqual(_resource_preference_details(profile)[0], {'lecture'})
        self.client.put('/personalization/learner-profile', json={'resource_preference': []})
        self.assertEqual(self.client.get('/training/onboarding/status').json()['survey_answers']['resource_preference'], [])
    ...
    def test_onboarding_submit_keeps_explicit_daily_minutes_while_filling_other_defaults(self):
        ...
        self.assertEqual(baseline["preferred_time_slot"], "碎片化不固定")
        self.assertEqual(baseline["resource_preference"], [])
    ...
    def test_onboarding_submit_preserves_learning_mode_and_drops_legacy_difficulty_preference(self):
        ...
        profile = self.client.get("/personalization/learner-profile")
        self.assertEqual(profile.status_code, 200)
        self.assertNotIn("问答优先", profile.json()["resource_preferences"])
        self.assertEqual(profile.json()["learning_habits"], "问答优先")
