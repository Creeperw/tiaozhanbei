...
_SHARED_RUNTIME_ROOT = Path(
    _env_or_default("SHIZHEN_RUNTIME_ROOT", str(_PROJECT_ROOT / "runtime"))
).expanduser().resolve()
_RUNTIME_DIR = Path(
    _env_or_default("BACKEND_RUNTIME_ROOT", str(_SHARED_RUNTIME_ROOT / "platform_backend"))
).expanduser().resolve()
_ASSET_ROOT = Path(
    _env_or_default("SHIZHEN_ASSET_ROOT", str(_PROJECT_ROOT / "assets"))
).expanduser().resolve()
_ASSET_RELEASE = _env_or_default("SHIZHEN_ASSET_RELEASE", "2026-07-18")
_KNOWLEDGE_DELIVERY_ROOT = Path(
    _env_or_default(
        "KNOWLEDGE_HANDOFF_ROOT",
        str(_ASSET_ROOT / "knowledge" / "releases" / _ASSET_RELEASE),
    )
).expanduser().resolve()
...
SQLITE_PATH = _env_or_default("SQLITE_PATH", str(_RUNTIME_DIR / "health_agent.db"))
...
# markitdown 提取产物预留目录。
MARKITDOWN_OUTPUT_DIR = _env_or_default(
...
KNOWLEDGE_ATLAS_ASSET_VERSION = _env_or_default("KNOWLEDGE_ATLAS_ASSET_VERSION", _ASSET_RELEASE)
KNOWLEDGE_ATLAS_DATA_ROOT = Path(
    _env_or_default(
        "KNOWLEDGE_ATLAS_DATA_ROOT",
        str(_KNOWLEDGE_DELIVERY_ROOT / "知识库管理组件" / "data" / "backend_delivery"),
    )
).resolve()
KNOWLEDGE_ATLAS_VIDEO_ROOT = Path(
    _env_or_default(
        "KNOWLEDGE_ATLAS_VIDEO_ROOT",
        str(_KNOWLEDGE_DELIVERY_ROOT / "bilibili_video_page" / "runtime"),
    )
).resolve()
...
class EmbeddingConfig:
    ...
    # 向量库根目录。
    BASE_DIR = _env_or_default(
        "VDB_STORE_ROOT",
        str(_ASSET_ROOT / "vectors" / "public" / _ASSET_RELEASE),
    )
    
    # 知识库源文件目录。
    DATA_SOURCE_PATH = _env_or_default(
        "KNOWLEDGE_DATA_SOURCE_PATH",
        str(_KNOWLEDGE_DELIVERY_ROOT / "知识库管理组件" / "data"),
    )
