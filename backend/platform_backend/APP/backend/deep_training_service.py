...
def select_practice_questions(
    *,
    ...
    retrieved = hybrid_rank_questions(
        question_bank,
        query="、".join(target_kp_ids),
        kp_ids=target_kp_ids,
        limit=len(question_bank) or 1,
    )
    ...
    def score(item: dict[str, Any]) -> float:
        ...
        retrieval = float(item.get("fusion_score") or 0.0)
        return 0.35 * retrieval + 0.30 * weak_match + 0.20 * target_match + 0.15 * quality

    selected = sorted(retrieved, key=score, reverse=True)[:limit]
    covered = set()
    ...
    selection = {
        ...
        "selection_policy": (
            "统一宽召回+RRF后，按0.35*检索相关性 + 0.30*薄弱点 + "
            "0.20*目标知识点 + 0.15*题目质量进行个性化约束选择"
        ),
    }
