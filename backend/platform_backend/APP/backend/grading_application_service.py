...
from APP.backend.knowledge_point_identity_service import canonical_map_for_ids
...
def apply_practice_grading(
    ...
    *,
    ...
    source_kp_ids = tuple(dict.fromkeys(
        str(kp_id).strip() for kp_id in command.kp_ids if str(kp_id).strip()
    ))
    canonical_map = canonical_map_for_ids(db, source_kp_ids)
    canonical_kp_ids = tuple(dict.fromkeys(
        canonical_map[source_id] for source_id in source_kp_ids
    ))
    ...
    item = LearningAttemptItemRecord(
        ...
        kp_snapshot_json=json.dumps([
            dict(
                ...
                **(
                    {"source_kp_ids": source_lineage}
                    if source_lineage != [kp_id]
                    else {}
                ),
            )
            for kp_id in canonical_kp_ids
            for source_lineage in [[
                source_id
                for source_id in source_kp_ids
                if canonical_map[source_id] == kp_id
            ]]
        ]),
        source_kind=command.source_channel,
    )
    ...
    evidence_payload = {
        "attempt_item_id": attempt_item_id,
        "question_version_id": command.question_version_id,
        "source_kp_ids": list(source_kp_ids),
    }
    evidence = EvidencePackRecord(
        pack_id=pack_id,
        user_id=command.learner_id,
        query=command.stem,
        resolved_kp_ids_json=json.dumps(canonical_kp_ids),
    ...
    grading = GradingResultRecord(
        ...
        kp_ids_json=json.dumps(canonical_kp_ids),
