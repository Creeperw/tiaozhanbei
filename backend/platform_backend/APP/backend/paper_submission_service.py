...
def _normalize_paper_scores(paper: PaperInstanceRecord, items: list[PaperItemRecord]) -> float:
    if not items:
        return 0.0
    current = [round(float(item.max_score_snapshot or 0.0), 2) for item in items]
    # Keep compatibility with the older malformed shape where every question
    # was persisted as 100 points. Normal papers use the sum of item scores;
    # this narrow legacy case is the only permitted normalization.
    if len(current) > 1 and all(abs(score - 100.0) <= 0.005 for score in current):
        current = _normalized_item_scores(
            [{"score": score} for score in current],
            {"total_score": 100.0},
            {},
        )
    for item, score in zip(items, current):
...
def _normalize_submission_result(
    ...
    normalized_result["score_rate"] = round(total / maximum * 100, 2) if maximum > 0 else 0.0
...
def submit_paper(
    ...
    *,
    ...
    try:
        ...
        total = round(total, 2)
        total = round(sum(float(item["score"]) for item in results), 2)
        maximum = round(sum(float(item["max_score"]) for item in results), 2)
        result = {"paper_id": paper_id, "status": "completed", "score": total, "max_score": maximum, "score_rate": round(total / maximum * 100, 2) if maximum > 0 else 0.0, "items": results}
