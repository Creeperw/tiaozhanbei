...
$Script:RepositoryRoot = (Resolve-Path (Join-Path $Script:Root "../..")).Path
$Script:FrontendRoot = (Resolve-Path (Join-Path $Script:RepositoryRoot "frontend/llm")).Path
$Script:PythonExe  = if ($env:BACKEND_PYTHON) {
    (Resolve-Path $env:BACKEND_PYTHON).Path
} else {
    (Get-Command python -ErrorAction Stop).Source
}
$Script:RuntimeRoot = if ($env:SHIZHEN_RUNTIME_ROOT) {
    [System.IO.Path]::GetFullPath($env:SHIZHEN_RUNTIME_ROOT)
} else {
    Join-Path $Script:RepositoryRoot "runtime"
}
$Script:PidDir     = Join-Path $Script:RuntimeRoot "launcher"
$Script:LogDir     = Join-Path $Script:PidDir "logs"
...
function Start-Backend {
    ...
    $env:BACKEND_HANDOFF_ROOT = if ($env:BACKEND_HANDOFF_ROOT) { $env:BACKEND_HANDOFF_ROOT } else { "platform_backend" }
    $env:API_PORT = "7860"
