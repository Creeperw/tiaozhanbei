...
- 2026-09-06 历史记录兼容展示已部署：登录后进入“个人数据”，展开页面下方
   “历史学习记录（含导入数据）”，可分页浏览答题、错题、掌握记录、历史计划、学习任务、
   每日任务及条目、专注、活动、会话消息、干预和智能体记录。
   历史会话不会混入当前助教对话，历史计划不会自动成为当前考试的正式路径。
   展示接口不迁移或改写这些记录，也不会将历史分数伪造为审核通过的成绩。
   合成演示记录不代表真实学习成果；每条历史计划保留原始来源。
