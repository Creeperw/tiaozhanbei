...
“中医执业医师考试”后，不应再次出现“请说明具体考试或专业方向”的同义追问。若用户为非医学专业且
没有说明报考途径，下一问应转为“规定学历 / 中医（专长） / 师承或确有专长”的必要单选，这是新的
路线约束，不是重复追问。
...
首次作答时，后端优先使用题库已有解析；没有解析则使用本次 Expert/批改结果生成 `question_explanation` 并写入 `QuestionVersionRecord.analysis`。以后再次遇到同一题目版本时直接返回缓存解析，此时 `explanation_source=question_version_cache`。前端应分别展示“本次批改”与“题目解析”。
...
交卷响应的 `items[]` 固定包含 `is_correct`、`score`、`max_score`、`submitted_answer`、`standard_answer`、`explanation`、`grading_analysis`、`mistake_ids`。答错题在发布时已建立 `PaperItem -> QuestionVersion -> QuestionKPLink` 权威链，审核通过后会写入统一错题记录，并出现在错题变式来源中；不能只保存页面上的对错状态。
