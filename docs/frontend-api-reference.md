...
错题列表与“可生成变式的错题”不是同一集合。`mistakes` 返回所有归属当前用户的错题；每项的 `variation_available` 仅表示其是否具备已审核题目版本、知识点关联和评分证据。不能变式的错题仍必须展示，并使用 `variation_reason` 说明原因。同一题再次答错会更新活动中的错题及最近作答证据，不因无法生成变式而丢弃记录。
...
作答页固定按“单选题、多选题、填空题、简答题”分组展示；`case_quiz` 归入简答题区并保留自身题型标识。返回试卷列表时前端停止本地倒计时间隔并清理当前作答视图，但服务端 `expires_at` 不暂停，重新进入仍按原考试截止时间计算，避免通过离开页面暂停正式考试。交卷成功后倒计时立即停止并显示已交卷状态。
...
当前 UI 使用的 `task_type`：`knowledge_card_generation`、`paper_generation`、`mistake_variation`。普通客观题和案例简答直接使用 `/api/v1/workshop/practice/*`；AI 病患模拟使用独立病例会话接口，不通过此字段伪装。
...
知识图谱的目录和画布必须使用同一套顺序：二级章节优先按后端 `order_index` 的教材原始顺序，三级知识点按后端返回的拼音顺序。前端筛选只移除节点，不得重新按标题或资源数量排序；否则目录与画布会出现同一层级顺序不一致。
...
| 个性数据 | `/api/v1/learning-context`、`/api/v1/learning-activity/*` | 画像、记忆编辑暂用 `/api/personalization*` |
