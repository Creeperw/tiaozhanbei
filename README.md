...
现有前端仍使用交接业务域的 Bearer Token 接口。体验完整前端时必须设置
`BACKEND_HANDOFF_ENABLED=true`，Vite 会把 `/api/*` 转发到 `8878` 并去掉 `/api` 前缀：
...
打开 `http://127.0.0.1:5173`。只验证主后端时仍保持 `BACKEND_HANDOFF_ENABLED=false`。
